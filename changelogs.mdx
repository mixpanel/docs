...
<Update label="2026-07-27">
  ...
  - **Automatically discover duplicates:** Mixpanel surfaces groups of near-duplicate events and event properties in your project — no manual searching required.
  - **Merge with a click:** Each suggested group comes with a canonical name (the most popular variant), so you can merge the whole group in a single click.
  - **Stay in control:** Merges behave exactly like a manual Lexicon merge — the raw data is unaffected and the action can be undone. Dismiss any suggestion you don't want and it won't show again.

  Magic Merge covers events and event properties. User (profile) properties cannot be merged at this time.
