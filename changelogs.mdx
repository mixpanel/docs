...
<Update label="2026-09-09">
  ## Agent Board Editing: change every report on a board at once

  Changing one filter across ten reports used to mean opening all ten and making the same edit ten times. The agent can now edit the reports already on a board, so a board-wide change takes a single request.

  With this update, you can:

  - **Change many reports in one pass:** Ask for an edit — a new filter, a different breakdown, an updated date range — and the agent applies it to every report on the board it affects, instead of you clicking into each one.
  - **Approve before anything changes:** The agent always presents a plan of the exact edits it intends to make. Nothing is touched until you approve it.
