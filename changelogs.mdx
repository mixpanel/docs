...
<Update label="2026-07-27">
  ## Magic Merge: automatically clean up duplicate events and properties

  Over time, the same concept often ends up tracked under several names — `Add to Cart`, `add_to_cart`, and `addToCart`, or `from_date` and `from-date`. These duplicates clutter Lexicon, split your data across reports, and make analysis harder. Magic Merge automatically discovers these potential duplicates for you, so you no longer have to hunt for them by hand.
  ...
  Magic Merge covers events and event properties (not user/profile properties) and is available to organizations on an Enterprise plan with Copilot enabled.

  [Learn more →](/docs/data-governance/data-clean-up#automatically-find-duplicates-with-magic-merge)
</Update>
