...
the **Storage Object Admin** (`roles/storage.objectAdmin`) role **on the destination bucket**.

Storage Object Admin is required because delivery overwrites objects: retried batches rewrite the same object name, and the access-verification probe is rewritten in place. Cloud Storage requires delete permission to overwrite an existing object, so object-creation permission alone is not sufficient.

<Note>
  Mixpanel only writes to your bucket. It never reads, lists, or deletes your existing data, even though Storage Object Admin technically permits it.

  If your security review requires a narrower grant, a custom role containing `storage.objects.create` and `storage.objects.delete` should suffice. **Storage Object Creator is not sufficient** — it cannot overwrite an existing object.
</Note>
