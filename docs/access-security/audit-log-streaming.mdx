...
<Steps>
  ...
  <Step title="Finish configuring the stream in Mixpanel">
    Return to Mixpanel and enter the S3 bucket details and IAM role ARN. Add an optional path prefix if you want the audit log objects written below a specific path, then save. Mixpanel verifies that it can assume the role and write to the bucket before creating the stream.
  </Step>
...
| "Mixpanel doesn't have permission to write to your destination." | AWS S3 | Confirm that the IAM role has `s3:PutObject` permission for `arn:aws:s3:::<BUCKET_NAME>/*` and that the policy names the correct bucket. |
| "Your destination couldn't be found." | AWS S3 | The bucket name or region is wrong, or the bucket has been deleted. Confirm the bucket details in AWS and update the stream. |
| "Mixpanel couldn't authenticate with your destination." | AWS S3 | Confirm that the role ARN is correct and its trust policy allows the Mixpanel export user to call `sts:AssumeRole` with the exact external ID shown in Mixpanel. |
