...
description: "Use SSO to access Mixpanel"
---
...
To access SSO settings in Mixpanel, navigate to your **Organization Settings** (gear icon on the bottom-left side of the screen -\> 'Settings' menu), and then click on the **Access Security** tab.
...
<Frame>
  ![SSO Access Security Image](/images/sso_access_security.png)
</Frame>

Click **Domain Claiming** in the Access Security menu.

<Frame>
  ![SSO Domain Claiming Image](/images/sso_domain_claiming.png)
</Frame>

Click **Add a Domain** found in the Domain Claiming sub-menu. You will be prompted to enter your Mixpanel password.

<Frame>
  ![SSO Add Domain Image](/images/sso_add_domain.png)
</Frame>

Enter the domain you wish to claim in the pop-up modal. Click **Submit Claim**.

<Frame>
  ![SSO Submit Claim Image](/images/sso_submit_claim.png)
</Frame>
...
The claimed domain is listed in the Domain Claiming menu. It will appear as pending until it is successfully verified.

<Frame>
  ![SSO Check Verification Status Image](/images/sso_check_verification_status.png)
</Frame>
...
You must configure your Identity Provider (IDP) to connect to Mixpanel to use SSO if you are not using custom-built SSO. This requires that you directly configure your SSO settings.
...
OneLogin only requires that you get the Postback URL. The "Mixpanel" application is in the OneLogin application store and supports auto-provisioning. You will just need to copy a SCIM token from Mixpanel into the provisioning token box in the OneLogin app. [Learn how to generate a token](/docs/access-security/single-sign-on#scim).

### G-Suite

[Google has an official integration with Mixpanel with instructions](https://support.google.com/a/answer/7553416). Unfortunately, we do not have an auto-provisioning integration with G-Suite. You will need to rely on [Just In Time Provisioning](/docs/access-security/single-sign-on#just-in-time-provisioning).

### Azure

Here are [instructions to set up SSO with Azure](/docs/access-security/single-sign-on/azure). Azure also has an [auto-provisioning integration with Mixpanel](https://learn.microsoft.com/en-us/azure/active-directory/saas-apps/mixpanel-provisioning-tutorial).

### Other IDPs

It is possible to set up Mixpanel SSO with IDPs not listed above. [Contact support](https://mixpanel.com/contact-us/support/) for further assistance in such cases.
...
<Frame>
  ![SSO Postback URL Image](/images/sso_postback_url.png)
</Frame>
...
- It's important to note that this certificate will expire after a certain number of years. At the moment, we do not send any notifications when it is about to expire. Please make sure you have a system set up to cycle your certificate every so often if you wish to avoid disruption.
...
## Require Single Sign-On for administrators
Optionally toggle on **Require Single Sign-On for administrators**; by default, even if the toggle to require SSO is enabled, organization admins and owners can still log in via email+password or magic link. In case you prefer to restrict everyone to use SSO, enable this toggle, and the exception to admins/owners will not apply. Do note that if your SSO is misconfigured, no one with administrator permissions can sign in to fix it, so it's important to verify your authentication is working before enabling.
...
- If they successfully log in through your SSO setup, they will be automatically added to the organization with no permissions except those granted to all users ([JIT provisioning](/docs/access-security/single-sign-on#just-in-time-provisioning)).
- It will also redirect anyone signing up for a Mixpanel account with your claimed domain or anyone requesting access to a project in your organization to log in via SSO first. This helps ensure all users in your organization who try to use Mixpanel get routed to your IDP, where you can then assign them access to the Mixpanel app. This also prevents needing to manually invite users to Mixpanel from within Mixpanel's Organization Settings.
...
<Frame>
  ![SSO IDP Managed Access Image](/images/sso_idp_managed_access.png)
</Frame>

## Just in Time Provisioning

Just-in-Time (JIT) provisioning using SAML will let users sign in automatically upon the initial login event. This removes the need for the organization admin to invite individual users to an Organization. This is part of the **IDP Managed Access** feature.
...
Users added using the IDP will have first names and last names populated by the `firstName` and `lastName` profile attributes provided via SAML at login time. These users will also have no roles to start off except those given to **all users** in your organization.

To give these provisioned users default access to projects, invite **All Users in the Organization** to the project.

<Frame>
  ![image](/images/sso_jit_all_users_invite.png)
</Frame>
...
<Frame>
  ![SSO Remove Configuration](/images/sso_remove_config.png)
</Frame>
