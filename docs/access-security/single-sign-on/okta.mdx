...
Before configuring Okta, [claim your domain](/docs/access-security/single-sign-on#claim-a-domain) in Mixpanel and get your [Postback URL](/docs/access-security/single-sign-on#postback-url) for use when configuring SAML in Okta.
...
- **Single sign on URL:** Postback URL from Mixpanel ([https://mixpanel.com/security/sso/v2/authorize/?org\_id=YOUR\_ORG\_ID](https://mixpanel.com/security/sso/v2/authorize/?org_id=YOUR_ORG_ID))
- **Requestable SSO URLs:** [https://sso.mixpanel.com/sso/saml2](https://sso.mixpanel.com/sso/saml2)
- **Recipient URL:** [https://sso.mixpanel.com/sso/saml2](https://sso.mixpanel.com/sso/saml2)
- **Destination URL:** [https://sso.mixpanel.com/sso/saml2](https://sso.mixpanel.com/sso/saml2)
- **Audience URI:** [https://mixpanel.com/security/sso/v2/authorize/](https://mixpanel.com/security/sso/v2/authorize/)

The following screenshot highlights what you should place in the fields:

<Frame>
  ![Okta Config SAML 1 Image](/images/okta_config_1.png)
</Frame>

To add the Requestable SSO URLs field, navigate to Advanced Settings:

<Frame>
  ![Okta Config SAML 2 Image](/images/okta_config_2.png)
</Frame>
...
<Frame>
  ![Okta Config SAML 2 Image](/images/okta_config_3.png)
</Frame>
...
<Frame>
  ![Okta Info 1 Image](/images/Admin/Okta/okta_saml_setup_instructions_fixed.png)
</Frame>
...
<Frame>
  ![Okta Info 2 Image](/images/okta_info2.png)
</Frame>
...
<Frame>
  ![Okta Certificate Download Image](/images/Admin/Okta/okta_download_certificate.png)
</Frame>
...
<Frame>
  ![Okta Info 3 Image](/images/okta_info3.png)
</Frame>
...
<Frame>
  ![Okta Info 4 Image](/images/okta_info4.png)
</Frame>
...
1. Collect the **Public Certificate**, **Authentication URL**, and **Issuer URL** as described above.
2. In Mixpanel, go to **Organization Settings** > **Access Security** > **Single Sign-On**.
3. Enter the **Public Certificate** in the **SAML Certificate** field, the **Authentication URL** in the **Identity Provider Sign-in URL** field, and the **Issuer URL** in the **Issuer URL** field.
4. Click **Update**.

Optionally, enable **Require Single Sign-On** to prevent users from logging in with a username and password. Organization Owners and Admins can still log in with a username and password if SSO is not set up correctly.
...
<Frame>
  ![Okta SCIM 1 Image](/images/Admin/Okta/sso_scim_token_updated_2024.png)
</Frame>
...
- You will need to provision other [Organization Roles](/docs/orgs-and-projects/roles-and-permissions#organization-roles) to users within the Mixpanel product.
...
<Frame>
  ![Okta SCIM 2 Image](/images/okta_scim2.png)
</Frame>
...
<Frame>
  ![Okta SCIM 3 Image](/images/okta_scim3.png)
</Frame>
...
<Frame>
  ![Okta SCIM 4 Image](/images/okta_scim4.png)
</Frame>

4. Select and assign the users you wish to provision:

<Frame>
  ![Okta SCIM 5 Image](/images/okta_scim5.png)
</Frame>
...
Mixpanel supports Okta's [Universal Logout](https://help.okta.com/oie/en-us/content/topics/itp/universal-logout.htm), which lets an Okta Admin trigger a logout from Okta that propagates to Mixpanel.

This functionality is available after enabling SSO.

#### OIN App

If you use the [Mixpanel app within the OIN (Okta's app store)](https://www.okta.com/integrations/mixpanel/), enable it by updating the Mixpanel application in the Okta Admin Console:

1. Log in to your organization's [Okta Admin Console](https://developer.okta.com/login/).
2. Select **Applications** from the **Applications** drop-down list in the left navigation drawer.
3. Select the **Mixpanel** app. The **Mixpanel Overview** page is displayed.
4. Activate the **Authentication** or **Sign On** tab.
5. Click **Edit** in the **Logout** section.
6. Select the **Okta system or admin initiates logout** checkbox.
7. Click **Save**.

#### Custom Apps

If you created a custom app, configure Universal Logout using the Global Token Revocation endpoint:

1. Log in to your organization's [Okta Admin Console](https://developer.okta.com/login/).
2. Select **Applications** from the **Applications** drop-down list in the left navigation drawer.
3. Select your custom Mixpanel app.
4. Activate the **General** tab.
5. In the **Logout** section, click **Edit**.
6. Under **Global Token Revocation**, select **Okta system or admin initiates logout**.
7. In the **Logout endpoint URL** field, enter `https://platform-us.mixpanel.com/v1/global-token-revocation`.
8. Set **Endpoint authentication type** to **Signed JWT**.
9. For **Subject format type**, select **Email Identifier**.
10. Click **Save**.
