---
title: AB Tasty
description: "Bidirectional data sync between Mixpanel cohorts and AB Tasty for targeted experimentation"
---
...
2. Click **Add Destination**, select AB Tasty, and click Connect.
...
<Tabs>
  <Tab title="From the Destinations page (recommended)">
    1. Navigate to **Data** in the nav bar, then select **Destinations**.
    2. Find the AB Tasty destination in the list and click on it.
...
- AB tasty pushes the abtasty\_visitor\_id to Mixpanel, and Mixpanel includes it in the cohort export. The abtasty\_visitor\_id is used to match the user.
...
> **How it works:** This formula reads the `"AB Tasty"` property and extracts only the text that exists _between_ the first and second ID brackets (e.g., `[12345]` and `[67890]`).
