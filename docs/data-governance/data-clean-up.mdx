...
Drop Filters let you permanently stop unwanted events from being ingested and stored into Mixpanel going forward. Where [Data Deletion](#2-deleting-problematic-data) addresses _past_ problematic data, Drop Filters address it _proactively_ — events matching your filter rules are dropped at ingestion and never stored, never queried, and never counted.
