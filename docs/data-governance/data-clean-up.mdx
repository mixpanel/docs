...
### Automatically find duplicates with Magic Merge
...
Magic Merge automatically discovers these potential duplicates in your project for you, so you don't have to hunt for them by hand. It groups near-duplicate **events** and **event properties** together and suggests merging each group into a single canonical name — which you can do with the click of a button.

<Info>
Magic Merge covers events and event properties only. User (profile) properties cannot be merged at this time.
</Info>

To use Magic Merge:

1. Open [Lexicon](/docs/data-governance/lexicon). Magic Merge surfaces its suggested groups of duplicate events and event properties.
2. Review a suggested group. Each group shows the near-duplicate names it found and the suggested canonical name to merge them into (the most popular variant in the group).
3. Click to merge the group. Mixpanel merges the duplicates into the canonical name, just like a [manual merge](/docs/data-governance/lexicon#merging-data) — the underlying raw data is unaffected, and the merge can be undone from Lexicon.
4. Dismiss any suggestion you don't want to act on, and it won't be shown again.

Because a merge behaves exactly like a manual Lexicon merge, the same considerations apply: after merging, update any reports or custom events that referenced the old names, and note that [Autocapture](/docs/tracking-methods/autocapture) events cannot be merged.
...
| I no longer want an event/ property entirely, as it’s no longer relevant to my team | No, we recommend you actually use [Dropping and Hiding Data](/docs/data-governance/lexicon#dropping-and-hiding-data) instead, as these will hide the events from your team, without using up your limited Data Deletion requests. |
| I don’t need to legally delete some events with sensitive data, but I don’t want some of my users to be able to see the sensitive data | No, we recommend you use [Mark Properties as Classified](/docs/data-governance/data-views-and-classification#mark-properties-as-classified-data) to remove the data from the analysis experience for certain user types, while keeping accessible for other users. |
| I want to delete data associated with a specific distinct ID | No, we recommend you use our [Distinct ID Deletion API](/reference/create-deletion), as Data Deletion does not support Distinct ID as a filtering criteria |
...
<Warning>
    While the deletion becomes permanent after 7 days, the process to fully clean the data from the storage can take up to 30 days total. During this time, said data will not appear in reports, or formatted API queries, but might appear in raw exports while it's being fully removed
</Warning>
...
A Drop Filter targets a specific event, narrowed by one or more property values. Every filter must include:

- **Event name** — the event you want to filter (required)
