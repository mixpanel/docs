...
description: "Set up a raw export pipeline from Mixpanel to a Google Cloud Storage bucket, with permissions and an example API request"
---
...
To set up a raw export pipeline to Google Cloud Storage (GCS) from Mixpanel, you must configure GCS to receive the exported data, then [create a pipeline](/reference/create-warehouse-pipeline) to export the data.
