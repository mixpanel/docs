---
sidebarTitle: Enterprise Group Analytics (independent)
title: "Enterprise Group Analytics"
description: "Define independent group keys to count uniques, build funnels, and measure retention at the group level"
---

<Note>
  **Looking for Group Analytics?** If you need an enforced Company → User hierarchy, see [Group Analytics](/docs/data-structure/group-analytics/hierarchical-groups) instead.
</Note>

Enterprise Group Analytics lets you establish event properties other than `distinct_id` as identifiers by which to analyze your data. By defining **group keys** — such as `company_id`, `account_id`, `project_id`, or `billing_id` — you can count uniques, build funnels, and measure retention at the group level rather than (or alongside) the individual user level.
...
<Note>
  User level `distinct_id` follows the rules for how `distinct_id` is set based on your identity management system. As a result the above may not always be true if `user_id` is not yet defined, or if you are using Original ID Merge or a non-ID-Merge project.
</Note>
...
You can define a maximum of 1, 3, or 6 group keys per project depending on your plan. Events can carry values for multiple group IDs and keys simultaneously.

### How Analysis Works

When analyzing by **Group**, Mixpanel counts uniques by `group_id` values, while User level analysis uses `user_id` values for uniqueness. There is no hard-coded relationship between groups and users in the `distinct_id`. As such, users can technically exist in multiple groups but still be counted as a single user at the User level.

## Limits

The limits below ensure Enterprise Group Analytics works correctly in Mixpanel:
...
- This add-on supports between 1, 3, and 6 group keys depending on your plan options.

## Prerequisites

**Enterprise Group Analytics only functions on data ingested after group keys are set up.** You must have the groups package enabled on the account and group keys defined in Project Settings before events will start showing up in Groups. Enterprise Group Analytics is not retroactive, meaning any data ingested into Mixpanel prior to setting up the group keys will not be associated with your groups even if that information already has group key and ID set on it.
...
Enterprise Group Analytics does **not** change how `distinct_id` or user identity works. Users are still identified by `distinct_id` as normal. Group keys provide an additional, independent axis for counting uniques — they do not replace or modify user identity. It is not possible to merge different group IDs together.

## Implementation

Enterprise Group Analytics can be implemented using Mixpanel's SDK methods, in addition to Data Warehouse Connectors and HTTP APIs directly.
...
<Frame>
  ![image](/images/group_setup_1.png "Add Group Key")
</Frame>
...
1. **Choose which group key will be used as the B2B Company Key.** This is the key for which [Company Profiles (B2B Company Analytics)](/docs/data-structure/group-analytics#company-profiles) will be generated.
...
<Frame>
  ![image](/images/group_setup_2.png "Associating events to groups")
</Frame>

While the pre and post authentication events are tied to the user [via ID merge](/docs/tracking-methods/id-management), only events ingested with the group key in it (in this case `company_id`) would be associated with the group. In this timeline, the journey of the user starts with the initial `page_view` event, but the journey of the group starts with the `login` event.
...
To create or update a [group profile](/docs/data-structure/group-analytics#group-profiles), similarly to user profile operations, at least one (1) profile property must be set. A set operation for a group profile that does not exist would create said profile, while it would update an existing profile.
...
<Frame>
  ![image](/images/data-group-id.png "Data Group ID")
</Frame>
...
<Frame>
  ![image](/images/engage-api-data-group-id.png "Engage API Data Group ID")
</Frame>
...
Company Analytics is an advanced offering within Group Analytics. Group Analytics focuses on the concept of there being multiple entities on which analysis can be done independently (eg. user ID, restaurant ID, driver ID, company ID, etc.). Company Analytics is specific to B2B Companies where you are likely to have a user ID and company ID. Here we focus on the idea that users belong to a company, and that company behavior is dependent on user behavioral activity. For instance, in SaaS companies, the health of an account is determined by how active the users of that account are.
