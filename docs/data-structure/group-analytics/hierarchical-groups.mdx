...
description: "A two-level Company → User data model for B2B products, using Mixpanel's reserved company and user ID properties"
---
...
**This is a project-creation-time decision.** You cannot convert an existing project on the independent model to the hierarchical model that Group Analytics uses, and you cannot switch a project back to the independent model once the hierarchical model is enabled. Choose your model before sending any data. Existing projects would need their data migrated to a *new* project on the hierarchical model.
...
Group Analytics can only be implemented via Data Warehouse Connectors or directly via the HTTP APIs. The groups methods in Mixpanel's SDKs only support the independent model.
...
3. **Map the column containing your user-level identifier** to **User ID**. This mapping is **required** on the hierarchical model (it is optional on the independent model).
...
#### Can I start on the independent model and switch to the hierarchical model later?
