...
<Note>
  The `$company_id` key is fixed and must be present on your events, but the display name shown in Mixpanel can be customized in Project Settings if your top-level grouping represents something other than a company.
</Note>
