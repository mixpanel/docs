...
You can define between 3 and 6 group keys per project depending on your plan options, and events can carry values for multiple group IDs and keys simultaneously.
...
**Standard Groups only functions on data ingested after group keys are set up.** You must have the groups package enabled on the account and group keys defined in Project Settings before events will start showing up in Groups. Standard groups are not retroactive, meaning any data ingested into Mixpanel prior to setting up the group keys will not be associated with your groups even if that information already has group key and ID set on it.
