...
title: "Lookup Tables"
description: "Enrich ingested data using a CSV"
---
...
We also provide an [API](/reference/replace-lookup-table) and [Warehouse Connectors](/docs/tracking-methods/warehouse-connectors#lookup-tables) to keep Lookup Tables synced on a recurring basis.
...
The CSV must be valid according to [RFC4180](https://datatracker.ietf.org/doc/html/rfc4180). See our [API reference](/reference/replace-lookup-table) for more specific details about how we parse CSVs.
...
<Steps>
  <Step title="Upload your Lookup Table CSV">
    ...
    <Frame>
      ![image](/images/lexicon-lookup-table.png "Lexicon Lookup Tables")
    </Frame>
  </Step>
  <Step title="Map the Lookup Table to an event or user profile property">
    ...
    <Frame>
      ![image](/images/lexicon-import-lookup-table.png "Lexicon Import Lookup Table Modal")
    </Frame>
  </Step>
</Steps>
...
<Steps>
  <Step title='Click the "..." button next to the breakdown property from your report and choose "Map Lookup Table"'>
    This breakdown property will be used as the ID that the Lookup Table join key will map to.

    <Frame>
      ![image](/images/map-ephemeral-lookup-table.png "Ephemeral Lookup Tables")
    </Frame>
  </Step>
  <Step title="Select a Lookup Table to map to the breakdown property">
    Alternatively, you may upload a new Lookup Table CSV for use locally in the report.

    <Frame>
      ![image](/images/create-ephemeral-lookup-table-modal.png "Ephemeral Lookup Table Modal")
    </Frame>
  </Step>
</Steps>

Lookup Tables can be replaced with a fresh copy, either via our UI, [API](/reference/replace-lookup-table), or [Warehouse Connectors](/docs/tracking-methods/warehouse-connectors#lookup-tables).
...
<Frame>
  ![image](/images/lookup-table-properties.png "Lookup Tables Properties")
</Frame>
...
<Frame>
  ![image](/images/lookup-table-insights.png "Lookup Table in Insights")
</Frame>
...
When replacing a lookup table using the [API](/reference/replace-lookup-table), you would need to pass the lookup table's ID as part of the URL in the API call. This `id` can be found in Lexicon under the Lookup Table's Details. See below screenshot as example:

<Frame>
  ![image](/images/lookup-table-id.png "Lookup Table ID")
</Frame>
