...
Do note that Mixpanel's default user properties are only updated when you send or update at least one property yourself (via [\$set](/reference/profile-set) or [\$set_once](/reference/profile-set-property-once) operations). Please, refer to this article to [understand how to send or update profile properties](/docs/data-structure/user-profiles).
...
| -------- | ------------ | ----------- | :-----------: | :-----------: | :-----------: | :-----------: |
| \$device_id | Device ID | Autogenerated ID that is local to the device. Calling reset() regenerates this value. More details [here](/docs/tracking-methods/id-management/identifying-users-simplified). | ✅ | ✅ | ✅ | ❌ |
| \$user_id | User ID | The identified ID of the user. Calling identify() sets this. More details [here](/docs/tracking-methods/id-management/identifying-users-simplified). | ✅ | ✅ | ✅ | ❌ |
