...
After an experiment is decided — a ship or kill decision has been recorded and its status is success or fail — you can reopen it in place instead of duplicating it. You don't need to create a new experiment to revisit the decision.
...
- **Undo Decision**: Revert the experiment to its concluded, pre-decision state so the decision can be revisited. The experiment returns to the point it was at before you recorded a decision.
...
Reopening or extending a decided experiment carries real risks, so weigh them before you do it:

- **Statistical validity**: Extending an experiment after you've already seen the results inflates the false-positive rate. Each time you look at the data and then decide to keep collecting, you give a non-significant result more chances to cross the threshold by chance, which makes a "significant" outcome less trustworthy.
- **Randomization and sample composition**: Reopening extends data collection into a new time window with a different mix of users. The population entering the experiment later may not match the earlier one (seasonality, marketing campaigns, or changes in who is being exposed), which can bias the comparison between variants.
- **Operational risk**: A decision that's already been recorded may have been communicated or acted on. Changing it after the fact can create confusion if teams have started to move on the original outcome.

If you want to test a different configuration or run the comparison again cleanly, restart the experiment instead of reopening it. Restarting resets the data so your analysis starts fresh, which avoids the peeking and sample-composition problems described above.
