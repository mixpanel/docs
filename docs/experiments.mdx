...
[No-code experiments](/docs/experiments/no-code-experiments) are run on a Mixpanel Feature Flag, so they are analyzed the same way, but you build their variants in a visual editor with no code changes required.
</Note>
