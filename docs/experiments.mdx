...
Once an experiment is decided (a ship or kill decision is recorded and its status is success or fail), you can reopen it in place instead of duplicating it.
...
Reopening or extending a decided experiment has trade-offs to consider first. Extending an experiment after you've already seen the results inflates the false-positive rate: each time you look at the data and then decide to keep collecting, you give a non-significant result more chances to cross the threshold by chance, which makes a significant outcome less trustworthy. Reopening also extends data collection into a new time window with a different mix of users. The population entering the experiment later may not match the earlier one (due to seasonality, marketing campaigns, or changes in who is being exposed), which can bias the comparison between variants. And a decision that has already been recorded may have been communicated or acted on, so changing it can create confusion if teams have started to move on the original outcome.

If you want to test a different configuration or run the comparison again cleanly, restart the experiment instead of reopening it. Restarting resets the data so your analysis starts fresh, which avoids the peeking and sample-composition problems above.
