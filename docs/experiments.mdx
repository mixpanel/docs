...
Click 'New Experiment' from the Experiment report menu and select your experiment. Both experiments powered by Mixpanel Feature Flags and experiments tracked via exposure events such as from third-party feature flags are detected and populated in the dropdown. To keep the dropdown focused on experiments you're likely to care about, we hide Mixpanel Feature Flag experiments once their flag is archived, and we hide exposure-event experiments that haven't received an exposure event in the last 30 days. To analyze an exposure-event experiment beyond that window, you can hard-code the experiment name by typing `$experiment:Experiment name` as a filter or breakdown property, where `Experiment name` is the value of whichever property you configured as the experiment name on your exposure events.

<Note>
Two kinds of experiments can be analyzed in the experiment report: 1) experiments run through Mixpanel Feature Flags with the Experiment flag type, and 2) experiments tracked via exposure events such as from third-party feature flags (i.e., `$experiment_started`). Read more on how to track experiments [here](#implementation-for-experimentation).

[No-code experiments](/docs/experiments/no-code-experiments) are the first kind. They run on a Mixpanel Feature Flag, so they are analyzed the same way, but you build their variants in a visual editor instead of in your codebase.
</Note>
