...
Enter either a target sample size or the minimum number of days you want the experiment to run. This determines the experiment’s analysis duration and when results are ready to review. Reaching the target does not stop the feature flag from serving; exposure continues according to the flag’s targeting and rollout settings until you change or disable the flag.
