---
sidebarTitle: "Export Methods"
title: "Export Methods: Export raw data and reports from Mixpanel"
---
...
<Note>
  For GDPR/CCPA-compliant exports, please [see the End User Data Management page](/docs/privacy/end-user-data-management).
</Note>
...
To export raw event data, use the [Raw Event Export API](/reference/raw-event-export). The events payload is returned as JSONL, where each line is a valid JSON object. This method is recommended for high volume exports.
...
<Frame>
![image](/images/event-csv-export-settings.png)
</Frame>
...
To export User Profile data, use the [Engage API](/reference/engage-query). The return payload is a JSON object, where the profiles are stored in the results key, as an array of objects where each profile is its own object.
...
You may export specific cohorts by using the `filter_by_cohort` parameter. You may find the `cohort_id` in the UI next to your cohort or by using the [cohorts-list API](/reference/cohorts-list).

<Frame>
![image](/images/cohort-id-viz.png)
</Frame>

#### Group Profile Export via API

You can also leverage the [Engage API](/reference/engage-query) to export Group Profiles. To do so, provide  the `data_group_id` parameter in your Engage API request. You can find the `data_group_id` for your group keys in your project settings.

<Frame>
![image](/images/data_group_id_project_settings.png)
</Frame>
...
<Frame>
![image](/images/switch_group_key.png)
</Frame>
...
You may use our [Query API](/reference/query-api) to export the formatted results that you see in our web app. Learn more about the [Insights API](/reference/insights-query), [Funnels API](/reference/funnels-query), and [Retention API](/reference/retention-query). 
