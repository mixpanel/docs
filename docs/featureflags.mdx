...
<Steps>
  ...
  <Step title="Runtime Targeting">
    ...
    **Filter by Runtime Property**
    ...
    Each condition has a **Data Type** that controls how Mixpanel interprets and compares the value. The value your application sends at request time (in the `custom_properties` object of the flag `context`) must be in the format expected for the chosen data type. If the value can't be parsed as that type, the condition will not match. The operators available in the condition adjust automatically to the selected data type.

    | Data Type | What your app should send | Matching behavior |
    | --- | --- | --- |
    | **String** | A string, e.g. `"enterprise"` | Case-insensitive string match. Comparison operators (Greater than, Less than, and their "or equal to" variants) compare strings lexicographically (character order), not numerically. Operators: Is, Is not, Contains, In, Greater than, Greater than or equal to, Less than, Less than or equal to |
    | **Number** | A number, e.g. `42` | Numeric comparison. Operators: Is, Is not, Greater than, Greater than or equal to, Less than, Less than or equal to |
    ...
    - **Fuzzy** is a case-insensitive loose-match type that coerces across types when comparing (for example, `"5"` matches `5`, and `"1"` matches `true`).
