...
<Steps>
  ...
  <Step title="Runtime Targeting">
    ...
    **Filter by Runtime Property**
    ...
    This method enables targeting users immediately based on arbitrary key/value attributes your application sends in the flag-evaluation request context (`context` / `custom_properties`) at request time. The flag compares these values directly (e.g. equals / contains) for immediate, per-request decisions.
    ...
    - These attributes should be passed to the SDK in the `context` / `custom_properties` parameter at the time the flag is evaluated.

    #### Data types for runtime values

    Each condition has a **Data Type** that controls how Mixpanel interprets and compares the value. The value your application sends at request time (in `context` / `custom_properties`) must be in the format expected for the chosen data type. If the value can't be parsed as that type, the condition will not match. The operators available in the condition adjust automatically to the selected data type.

    | Data Type | What your app should send | Matching behavior |
    | --- | --- | --- |
    | **String** (default) | A string, e.g. `"enterprise"` | Case-insensitive string match. Operators: `is`, `is not`, `contains`, `in`, `<`, `<=`, `>`, `>=` |
    | **Number** | A number, e.g. `42` | Numeric comparison. Operators: `is`, `is not`, `<`, `<=`, `>`, `>=` |
    | **True/False** | A boolean, `true` or `false` | Boolean match. Operators: `is True`, `is False` |

    - **Lists**: if the value your app sends is a list, you can require that **any** or **all** elements satisfy the condition. Each element is compared using the element's data type according to the rules above.
    - **Fuzzy** is a legacy, case-insensitive loose-match type retained for conditions authored before typed matching existed (for example, `"5"` matches `5`, and `"1"` matches `true`). It is not offered when creating new conditions. Prefer an explicit data type instead.
