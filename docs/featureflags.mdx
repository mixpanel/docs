...
- **Runtime Properties** — any key/value attributes your application passes in the flag-evaluation request context (`context` / `custom_properties`) at request time; the flag compares these values directly (e.g. equals / contains). They are not limited to device or platform values — any attribute your app can supply at request time, such as `organization_id`, works, making them the right tool for use cases like organization- or account-based targeting.
...
- Runtime Properties are **not** limited to device or platform values. They can be any attribute your frontend or app supplies at request time — for example, `platform`, `path`, `country`, or a custom value like `organization_id`. This makes them the right tool for use cases such as organization- or account-based targeting, in addition to device/platform targeting. For example, only users who are on Android devices and in the UK, or only requests belonging to a specific organization.
