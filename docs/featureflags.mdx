...
- **Fallback Value** — the value your app supplies at call time (e.g., to `get_variant_value`) that is used whenever the SDK does not return an assignment. This is the value served to any user the rollout doesn't reach — someone outside the Rollout %, excluded by targeting filters, or affected by feature-flagging service downtime — so treat it as the expected default experience for everyone not explicitly rolled out, not just a downtime safeguard.
...
<Note>
  Service downtime is only one case where the fallback value is served — it's also what's returned for every user who isn't covered by the rollout, e.g. anyone outside the Rollout %, or excluded by targeting filters. In a 50% rollout, the other 50% of users see the fallback value on every normal request, not just during an outage. Set it to the default experience you want those users to have.
</Note>
