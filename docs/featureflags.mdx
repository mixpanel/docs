...
| Plan | Active Flag Allowance | Cohort Targeting | How to Access |
| --- | --- | --- | --- |
| Free | 10 | Cohorts up to 100k users, in one rollout group | Switch plans from the [pricing page](https://mixpanel.com/pricing/). |
| Growth | 50, or 500 with purchased Experiments volume | Cohorts up to 20M users | Purchase Experiments volume under Organization Settings \> Plan Details & Billing. |
| Enterprise | 3, or 500 with purchased Experiments | Cohorts up to 20M users | Purchase Experiments to raise your limit to 500 active flags — [contact us](https://mixpanel.com/contact-us/sales/) or reach out to your account team. |

The Enterprise allowance of 3 active flags is a trial amount for evaluating Feature Flags; purchasing Experiments raises it to 500.
...
{/* ============================================================ */}
  ...
  <Step title="Runtime Targeting">
    ...
    **Filter by Runtime Property**
    ...
    - Runtime Properties are **not** limited to device or platform values. They can be any attribute your frontend or app supplies at request time — for example, `platform`, `path`, `country`, or a custom value like `organization_id`. This makes them the right tool for **immediate**, per-request targeting, such as organization- or account-based targeting where you need decisions to take effect right away. If a periodic cohort refresh (approximately every 2 hours) is acceptable, [group cohort targeting](#rollout-groups) is an alternative approach for org/account-based use cases.
...
| **Semantic Version** | A version string, e.g. `"2.1.0"` or `"2.5.0-rc.1"` | Ordered by the [SemVer 2.0.0](https://semver.org/) specification i.e. major, then minor, then patch, with a pre-release ranking below the release it belongs to. A leading `v` is dropped, a partial version such as `1.2` is read as `1.2.0`, and build metadata after `+` is ignored. | <ul><li>Is</li><li>Is not</li><li>Greater than</li><li>Greater than or equal to</li><li>Less than</li><li>Less than or equal to</li><li>Between</li></ul> |
    ...
    <Warning>
      **Applies to Local Evaluation and Runtime Event filters.** Date and Semantic Version are compared inside the SDK in these two cases — a server side SDK using [Local Evaluation](#local-evaluation), or a client side SDK evaluating a [Runtime Event](/docs/featureflags/runtime-events) filter on the device — so both require a recent SDK version. Check the minimum version on your SDK's feature flag guide under [Implementation](#implementation) before you use either type. If an SDK version is older than the minimum required, the condition will NOT match, so no one is placed in that rollout group.

      React Native and Flutter do not support Date or Semantic Version in Runtime Event filters yet. Remote Evaluation is unaffected, since Mixpanel performs the comparison.
    </Warning>
