...
Mixpanel Feature Flags let you control who sees a feature and when — gradual rollouts, cohort targeting, kill switches, and variant delivery for experiments.

## Plan Availability

Feature Flags is available for customers on Free, Growth, and Enterprise plans. It comes bundled with [Experiments](/docs/experiments) at no extra cost, and you can use flags on their own — you don't have to create or run an experiment. Each plan includes a set number of active flags: the number of flags you can have **Enabled** at once, counted across your organization. There is no limit on the total number of flags you create, and no limit on API requests. See our [pricing page](https://mixpanel.com/pricing/) for more details.
...
Enterprise customers who need Feature Flags on a contract that excludes Experiments can [contact us](https://mixpanel.com/contact-us/sales/) about a standalone arrangement, which is priced on Feature Flag API requests.
  ...
  <Step title="Rollout Groups">
    ...
    **Group Cohort Targeting**
    ...
    <Note>
      ...
      - For the optimal performance, cohorts used in feature flag targeting are officially limited to 20 million users on Growth and Enterprise plans, and 100k users on Free plans.
      - On a Free plan, you can use cohort targeting conditions in one rollout group, and you cannot save cohorts for reuse.
    </Note>
