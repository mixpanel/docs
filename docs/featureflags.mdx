---
title: "Feature Flags"
description: "Rollout with precision control"
sidebarTitle: "Feature Flags"
---

Ship a feature to a subset of users, roll it back without a deploy, and measure what it changed — all from the same project your analytics already live in.

## Plan Availability

Feature Flags is available for customers on Free, Growth, and Enterprise plans, and is included with [Experiments](/docs/experiments) on every plan. You can use flags on their own — you don't have to run an experiment.

Each plan limits your **active flags**: flags marked Enabled, counted across your organization. Creating flags is unlimited. See our [pricing page](https://mixpanel.com/pricing/) for more details.

| Plan | Flag Allowance | Cohort Targeting | How to Access |
| --- | --- | --- | --- |
| Free | Up to 10 flags simultaneously active | Cohorts up to 100k users, in one rollout group | Switch plans from the [pricing page](https://mixpanel.com/pricing/). |
| Growth | Up to 50 flags simultaneously active, or 500 with purchased Experiments volume | Cohorts up to 20M users | Purchase Experiments volume under Organization Settings > Plan Details & Billing. |
| Enterprise | Up to 3 flags created, or 500 simultaneously active with purchased Experiments | Cohorts up to 20M users | Purchase Experiments to raise your limit to 500 active flags — [contact us](https://mixpanel.com/contact-us/sales/) or reach out to your account team. |

The Enterprise allowance of up to 3 flags created is the default Enterprise trial amount for evaluating Feature Flags; purchasing Experiments removes this limitation and allows for up to 500 flags simultaneously active. [Contact us](https://mixpanel.com/contact-us/sales/) to inquire about an unlimited usage Enterprise trial.

Enterprise customers who need Feature Flags without Experiments can [contact us](https://mixpanel.com/contact-us/sales/) about a standalone arrangement, which is priced on Feature Flag API requests. On every other plan, API requests are unlimited.
...
<Steps>
  <Step title="Variant Assignment Key">
  ...
  <Step title="Variants Management">
...
{/* ============================================================ */}
    ...
    <Note>
      ...
      - For the optimal performance, cohorts used in feature flag targeting are officially limited to 20 million users on Growth and Enterprise plans, and 100,000 users on Free plans.
      - On a Free plan, you can use cohort targeting conditions in one rollout group, and you cannot save cohorts for reuse.
    </Note>
  ...
  <Step title="Runtime Targeting">
    ...
    **Filter by Runtime Property**
    ...
    This method enables targeting users immediately based on arbitrary key/value attributes your application sends as properties in a `custom_properties` object nested within the flag-evaluation `context` at request time. The flag compares these values directly (e.g. equals / contains) for immediate, per-request decisions.

    - Runtime Properties are **not** limited to device or platform values. They can be any attribute your frontend or app supplies at request time — for example, `platform`, `path`, `country`, or a custom value like `organization_id`. This makes them the right tool for **immediate**, per-request targeting, such as organization- or account-based targeting where you need decisions to take effect right away. If a periodic cohort refresh (approximately every 2 hours) is acceptable, [group cohort targeting](#targeting-&-identity-management) is an alternative approach for org/account-based use cases.
    - These attributes should be passed to the SDK as properties in a `custom_properties` object that is a sub-node of the feature flag `context`, at the time the flag is evaluated.

    #### Data types for runtime values

    Each condition has a **Data Type** that controls how Mixpanel interprets and compares the value. The value your application sends at request time (in the `custom_properties` object) must be in the format expected for the chosen data type. If the value can't be parsed as that type, the condition will not match. The operators available in the condition adjust automatically to the selected data type.

| Data Type | What your app should send | How it matches | Operators |
| --- | --- | --- | --- |
| **String** | A string, e.g. `"enterprise"` | Case-insensitive text match for all operators. Ordered comparisons such as Greater Than, Greater than or equal to, Less than, or Less than or equal to compare strings in lexicographical order.| <ul><li>Is</li><li>Is not</li><li>Contains</li><li>In</li><li>Greater than</li><li>Greater than or equal to</li><li>Less than</li><li>Less than or equal to</li></ul> |
| **Number** | An integer or floating point number, e.g. `42`, `23.54` | Numeric comparison. | <ul><li>Is</li><li>Is not</li><li>Greater than</li><li>Greater than or equal to</li><li>Less than</li><li>Less than or equal to</li></ul> |
| **True/False** | A boolean, `true` or `false` | Boolean match | <ul><li>is True</li><li>is False</li></ul> |
| **List** | A list of values, e.g. `["a", "b"]` | Requires any or all elements to satisfy the condition. Each element is compared using its own data type, following that type's rules above. | <ul><li>any</li><li>all</li></ul> |
| **Fuzzy** | Any primitive value | Case-insensitive loose match that coerces across types before comparing, so `"5"` matches `5` and `"1"` matches `true`. | <ul><li>Is</li></ul> |
| **Date** | Must be a valid RFC3339 timestamp, e.g. `"2026-07-16T14:30:00Z"`. Other formatted timestamps will NOT match. | Compared as a point in time. The date you select in the rule is interpreted in your project's timezone, and a date with no time of day covers the whole day. | <ul><li>On</li><li>Not on</li><li>Before</li><li>Since</li><li>Between</li></ul> |
| **Semantic Version** | A version string, e.g. `"2.1.0"` or `"2.5.0-rc.1"` | Ordered by the [SemVer 2.0.0](https://semver.org/) specification i.e. major, then minor, then patch, with a pre-release ranking below the release it belongs to. A leading `v` is dropped, a partial version such as `1.2` is read as `1.2.0`, and build metadata after `+` is ignored. | <ul><li>Is</li><li>Is not</li><li>Greater than</li><li>Greater than or equal to</li><li>Less than</li><li>Less than or equal to</li><li>Between</li></ul> |

    <Warning>
      **Scope of Date and Semantic Version Operator Filters**

      Date and Semantic Version are compared inside the SDK in two cases: a server side SDK using [Local Evaluation](#local-evaluation), or a client side SDK evaluating a [Runtime Event](/docs/featureflags/runtime-events) filter on the device. Both require a recent SDK version. Check the minimum version on your SDK's feature flag guide under [Implementation](#implementation) before you use either type. If an SDK version is older than the minimum required, the condition will NOT match, so no one is placed in that rollout group.
    </Warning>

    <Note>
      **Runtime Property** (targeting based on values your app _sends_ in the request context) is distinct from **Runtime Event** (targeting based on Mixpanel events the user _triggers_ via `mixpanel.track()`). See the section below for details on Runtime Event filters.
    </Note>
...
**Are there any limits to the number of variants per flag?**

Yes, today you can have a maximum of 10 variants per flag.
...
>
> When you add a user as a QA tester (by email or distinct\_id), Mixpanel saves a snapshot of that user's **most recent `$identified_id`** at the moment of addition, pulled from their `$identify` event history. At flag evaluation time, the `/flags/` endpoint performs an exact string match between the distinct\_id your SDK sends and this saved value — there is no cluster-aware resolution at runtime.
>
...
- In case of Feature flagging service downtime, your fallback variant will be served by the SDKs. See [Fallback Value](#concepts--terminology) for more details about fallback values.
...
<Note>
  Flag persistence is a client SDK feature. It is officially supported on the Android, Swift, Flutter, React Native, and JavaScript (Web) client SDKs.
</Note>
...
| Policy | Behavior |
| --- | --- |
| `networkOnly` _(default)_ | No persistence of variants. Every lookup waits for a fresh network response. |
...
<CardGroup cols={3}>
  <Card title="Web" href="/docs/tracking-methods/sdks/javascript/javascript-flags">
  </Card>

  <Card title="iOS" href="/docs/tracking-methods/sdks/swift/swift-flags">
  </Card>

  <Card title="Android" href="/docs/tracking-methods/sdks/android/android-flags">
  </Card>

  <Card title="Flutter" href="/docs/tracking-methods/sdks/flutter/flutter-flags">
  </Card>

  <Card title="React Native" href="/docs/tracking-methods/sdks/react-native/react-native-flags">
  </Card>
</CardGroup>
...
<CardGroup cols={3}>
  <Card title="Python" href="/docs/tracking-methods/sdks/python/python-flags">
  </Card>

  <Card title="Ruby (Beta)" href="/docs/tracking-methods/sdks/ruby/ruby-flags">
  </Card>

  <Card title="Node.js" href="/docs/tracking-methods/sdks/nodejs/nodejs-flags">
  </Card>

  <Card title="Java" href="/docs/tracking-methods/sdks/java/java-flags">
  </Card>

  <Card title="Go" href="/docs/tracking-methods/sdks/go/go-flags">
  </Card>
</CardGroup>
...
<CardGroup cols={3}>
  <Card title="Web" href="/docs/tracking-methods/sdks/javascript/javascript-openfeature">
  </Card>

  <Card title="iOS" href="/docs/tracking-methods/sdks/swift/swift-openfeature">
  </Card>

  <Card title="Android" href="/docs/tracking-methods/sdks/android/android-openfeature">
  </Card>
</CardGroup>
...
<CardGroup cols={3}>
  <Card title="Python" href="/docs/tracking-methods/sdks/python/python-openfeature">
  </Card>

  <Card title="Ruby" href="/docs/tracking-methods/sdks/ruby/ruby-openfeature">
  </Card>

  <Card title="Node.js" href="/docs/tracking-methods/sdks/nodejs/nodejs-openfeature">
  </Card>

  <Card title="Java" href="/docs/tracking-methods/sdks/java/java-openfeature">
  </Card>

  <Card title="Go" href="/docs/tracking-methods/sdks/go/go-openfeature">
  </Card>
</CardGroup>
...
**Mixpanel Feature Flags with a Customer Data Platform (CDP)** You have two options -
...
**Mixpanel Feature Flags with Google Tag Manager (GTM)** You can implement Mixpanel Feature Flags through Google Tag Manager. See the [Google Tag Manager integration guide](/docs/tracking-methods/integrations/google-tag-manager) for step-by-step instructions on initializing flags, evaluating them, and tracking exposure events via GTM.
...
Feature Flag rollouts may include configuration that references data groups in a given project. For instance, if you change the "Assignment Key" of a flag to a data group key, both the source and destination projects _must_ include the referenced data group in order to duplicate or push changes. If the assignment key is _not_ present in the destination project, the operation will fail.
...
#### How is Feature Flags priced?

Feature Flags is included with [Experiments](/docs/experiments) on every Mixpanel plan. Usage is limited by the number of active flags and the maximum cohort size for targeting — see [Plan Availability](#plan-availability). When Feature Flags is included with Experiments, API requests are unlimited. Standalone Feature Flags, purchased without Experiments on an Enterprise plan, is priced on Feature Flag API requests.

#### What counts as an active flag?

Active flags are flags which are marked **Enabled**. Only these flags are counted under your plan's active flag limit. There is no limit on the total number of flags you create.

#### What happens when I reach my active flag limit?

No more flags are fetched as part of the API request until you disable some others or upgrade your plan. Mixpanel fetches the flags with the earliest start dates, up to your limit; flags started after that are not returned. A flag's start date is the date it was first enabled.

#### Does the same flag count twice if I create it across projects?

No. Limits are based on active feature flag keys. If the same flag key is used across projects, it is counted only once.

#### How can I monitor my account’s Feature Flags usage?
...
A feature flag API request is a call made by your application to a feature flag system's API to determine which features should be enabled or disabled, or which variant should be served to a user. All the active flags in the system are evaluated as part of this API request.

#### How can I estimate Feature Flag API requests?

API requests are unlimited when Feature Flags is included with Experiments, but the estimate is useful for capacity planning, and it determines the price of a standalone Enterprise arrangement.

In general, you can expect every user session to have 1 API request. When the session starts, all the flags for the user are fetched in the single API request, so monthly API requests are at minimum equivalent to the total user sessions that month. Two notes:

- If you have multiple SDK inits each session, that increases the count of API requests in a session.
- As a general rule of thumb, consider API requests to be 1.5 x user sessions, to account for implementation differences and multiple SDK inits in a session.

For a server-side SDK:

- With local evaluation only, your API requests depend on how many server instances you have and how frequently you poll.
- With remote evaluation, or when using cohorts, your estimate is similar to the client-side SDK estimate above.

#### Can I purchase Feature Flags without Experiments?

Standalone Feature Flags is available to Enterprise plan customers. [Contact us](https://mixpanel.com/contact-us/sales/) for more details.
