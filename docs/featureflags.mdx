...
- **Fallback Value** — the value your app supplies at call time (e.g., to `get_variant_value`) that is used whenever the SDK does not return an assignment. This is what's served to a user who has never received an assignment — someone currently outside the Rollout %, excluded by targeting filters, or affected by feature-flagging service downtime — so treat it as the default experience for anyone the rollout hasn't reached, not just a downtime safeguard. It is not guaranteed for every user outside the current rollout, though: a user who was previously assigned a variant can keep seeing it even after a later change to rollout % or targeting would otherwise exclude them, because Sticky Variants and persisted client-side assignments retain prior variants instead of falling back.
...
<Note>
  Service downtime is only one case where the fallback value is served — it's also what's returned for a user who has never been assigned a variant, e.g. someone currently outside the Rollout %, or excluded by targeting filters. Set it to the default experience you want those users to have. This isn't guaranteed for every user outside a rollout, though: if a user was already assigned a variant, Sticky Variants and persisted client-side assignments will keep serving that prior variant even after a later change to rollout % or targeting would otherwise exclude them, rather than switching them to the fallback.
</Note>
