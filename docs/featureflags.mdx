...
<Note>
  **Runtime Property** (targeting based on values your app *sends* in the request context) is distinct from **Runtime Event** (targeting based on Mixpanel events the user *triggers* via `mixpanel.track()`). See the section below for details on Runtime Event filters.
</Note>
