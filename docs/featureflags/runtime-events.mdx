...
<Note>
  These property filters apply to properties of the selected runtime **event** — they narrow which occurrences of the tracked event count toward activation. If you want to target based on a value your application sends at request time (for example, an `organization_id`), use a [Runtime Property](/docs/featureflags#runtime-properties) filter instead.
</Note>
</Step>
