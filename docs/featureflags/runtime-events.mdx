---
sidebarTitle: Runtime Events
title: "Runtime Events"
description: "Target Users Based on Behavior"
---

<Note>
  Runtime Events is part of [Feature Flags](/docs/featureflags), which is available on all Mixpanel plans with limits that vary by plan. See [Feature Flags plan availability](/docs/featureflags#plan-availability) and our [pricing page](https://mixpanel.com/pricing/) for more details.
</Note>

## Overview

**Runtime Events** let you target users immediately based on actions they perform, not just who they are. Instead of targeting [cohorts](/docs/users/cohorts) or [properties](/docs/featureflags#runtime-targeting), you can trigger feature variants when users complete specific events tracked with `mixpanel.track()`.
...
<Steps>
  ...
  <Step title="(Optional) Add Property Filters">
    ...
    Each property filter has a **Data Type** (String, Number, or True/False) that controls how the event property value is compared. The value tracked on the event must be in the format expected for the chosen type, or the filter will not match. See [Data types for runtime values](/docs/featureflags#data-types-for-runtime-values) for the expected formats and operators.

    <Note>
      These property filters apply to properties of the selected runtime **event**. They narrow which occurrences of the tracked event count toward activation. If you want to target based on a value your application sends at request time (for example, an `organization_id`), use a [Runtime Property](/docs/featureflags#runtime-targeting) filter instead.
    </Note>
  </Step>
