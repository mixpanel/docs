...
Note: For Enterprise accounts using [Custom Roles](/docs/orgs-and-projects/roles-and-permissions#custom-roles), the create and manage warehouse metrics permissions are available to users with the Create and Manage Data Warehouses permission. 
...
## Editing a warehouse metric

Once a metric is saved, you can reopen the SQL editor from either of two places:

- **From an Insights report:** click the three dots on the metric, then select **Edit Metric**.
- **From Lexicon:** open the metric in [Lexicon](/docs/data-governance/lexicon) and click **Edit**.

Either path opens the same editor you used to create the metric, where you can revise the query and its settings.
...
- **Timeseries metrics** are plotted over time using the time column you selected in line and column charts. Time buckets with no matching rows show as 0.
