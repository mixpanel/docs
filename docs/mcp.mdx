...
<Note>
  **New free and growth accounts (created after August 1, 2026):** MCP is enabled by default — no admin action required. Organization admins can disable it in **Settings → Org → Overview** if needed. Free plans can be subject to Query API access limits, please refer to our [pricing page] (https://mixpanel.com/pricing/) for more information.  
...
<Tip>
  The [Mixpanel plugin for Claude Code](#claude-code-plugin) adds skills on top of this connection — guided workflows for tracking implementation, metric investigation, dashboards, and experiments — and can set up the MCP connection for you.
</Tip>
...
**Note:** If your workspace is in the **EU or IN region** **or** you want to publish a custom plugin to your workspace for teammates to use, you must first enable **Developer Mode**.

Go to **Settings → Security and login**, then enable **Developer Mode**.

Once **Developer Mode** is enabled, add a new plugin.

<Frame>
  ![Image](/images/image-8.png "Image")
</Frame>

1. Go to Plugins **→** add a New Plugin — set the [MCP Server URL](#mcp-server-urls) to your regional endpoint and Authentication to **OAuth**
2. Complete the Mixpanel OAuth flow
3. Optionally publish the connector to your workspace so teammates can use it
...
The [Mixpanel plugin for Claude Code](https://github.com/mixpanel/ai-plugins) adds a set of skills — guided workflows for the analytics work Mixpanel customers do most. The skills work through whichever engine you set up: the MCP server (any region) or the [`mixpanel-headless`](/docs/mixpanel-headless) Python SDK.

**Install the plugin**, then run the install skill once per project to set up an engine and region:
...
If you already connected the MCP server (see [Claude Code](#claude-code) above), the skills use that connection automatically — no extra setup.

### Included Skills

Claude invokes skills automatically based on your request, or you can invoke one directly with `/mixpanel:skill-name` (for example, `/mixpanel:deep-research`).

| Skill | What It Does |
| --- | --- |
| `install` | Sets up a Mixpanel engine for a project — MCP server (US/EU/India), mixpanel-headless SDK, or your own integration |
...
The plugin's skills are open source at [github.com/mixpanel/ai-plugins](https://github.com/mixpanel/ai-plugins). When the engine is the MCP server, the same [Permissions & Access](#permissions--access) rules apply — MCP must be enabled for your organization, and Claude can only see projects your Mixpanel user can access.
