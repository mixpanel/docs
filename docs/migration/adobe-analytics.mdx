...
<Frame>
  ![image](/images/adobe-vs-MP-instrumentation.png)
</Frame>
...
You’ll have to setup Mixpanel fresh given Mixpanel leverages an event-based model that does not exist in Adobe Analytics. Depending on your current setup and how you’ve enabled data capture in Adobe, these steps will likely be as if you are starting an [entirely new implementation](/docs/what-to-track).
...
- Configure the [Mixpanel JavaScript SDK](https://developer.mixpanel.com/docs/javascript-full-api-reference) to automatically track page views, where UTM params are tracked by default
  ...
  ```
- Track a few key [“value moment” events](/docs/what-to-track#choose-a-value-moment) of interest
...
**_This step needs to be done before any data is sent to the project._**
