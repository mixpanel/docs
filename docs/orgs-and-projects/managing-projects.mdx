...
<Note>
  Resetting a project does not remove the billing tallies it accumulated during the current billing month. Usage already counted toward your billing month is retained.
</Note>
...
Project Token cannot be reset in a self-serve manner. If you need to reset your project token for any reason, please [reach out to our Support team](https://mixpanel.com/get-support). After rotating it, the original one becomes invalid, so you will need to replace it with the new one in your implementation's code. Otherwise, no new events will be sent to your Mixpanel project.
