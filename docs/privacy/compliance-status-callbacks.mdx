...
| --- | --- | --- |
| `status_callback_urls` | No | A JSON array of HTTPS URLs. A non-HTTPS entry is rejected with a 400. |
...
`subject_request_id` is required with callbacks because OpenDSR reserves minting it to you, the controller — a callback keyed on an identifier you never chose is one you cannot correlate. It must be unique within your project.

It is echoed on every callback and on the status endpoint's response, so it is the identifier to correlate on rather than Mixpanel's `tracking_id`.
...
Your endpoint should return a `2xx`. Any other response, a timeout, or a TLS failure is treated as a failed delivery and retried.
...
Keys are serialized in sorted order with no whitespace, exactly as shown. See [Verifying the signature](#verifying-the-signature) for why that matters.

| Field | Type | Notes |
| --- | --- | --- |
| `api_version` | string | Always `3.0`. The DSR API version, not the version of the OpenDSR specification. |
...
<Warning>
  ...
  Mixpanel signs the exact bytes it sends: keys sorted, no whitespace between tokens. If your framework parses the JSON and you re-serialize it to verify, you will produce `{"a": 1}` where Mixpanel signed `{"a":1}`, and every callback will look forged.

  Most frameworks parse before your handler runs. Use `request.get_data()` in Flask, `express.raw()` or a saved raw buffer in Express, `request.body` in Django, and a `ContentCachingRequestWrapper` in Spring.
</Warning>
...
The first callback arrives shortly after you create the request, while it is still `pending`. Treat it as an integration check: within one delivery cycle you learn whether your endpoint is reachable and whether your signature verification works against a real body.
...
**When Mixpanel stops.** Retries stop 90 days after the request was created. After that you receive nothing further and the status endpoint is the only answer. This is longer than the 30-day `expected_completion_time` on purpose, so that a request which misses its deadline can still deliver its completion callback.

The status endpoint remains the source of truth throughout. If you have not heard from a callback you expected, poll it.
...
`api_version` is the DSR API version — the same value callbacks carry — not the version of the OpenDSR specification.
