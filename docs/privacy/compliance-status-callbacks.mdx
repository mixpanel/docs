...
| `results_url` | string or null | The signed export link on a completed retrieval, valid for 14 days from completion. `null` on deletions, on any non-completed status, and on a retrieval whose export has aged out. |
