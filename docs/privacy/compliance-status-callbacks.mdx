...
Return `202 Accepted` once the signature validates, and `401 Unauthorized` if it does not. Mixpanel treats any `2xx` as delivered; any other response, a timeout, or a TLS failure is a failed delivery and is retried.
...
| Field | Type | Notes |
| --- | --- | --- |
| `api_version` | string | Always `3.0`. Mixpanel's DSR API version. The discovery document's field of the same name means something else — see [Discovery](#discovery). |
