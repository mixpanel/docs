...
1. Navigate to "Data & Privacy" by clicking on the settings gear in the lower-left corner and navigating to Organization Settings.
...
| Sources | All five files, with the columns in events.csv and the entries in event_properties.csv narrowed to the properties that describe how the data was collected, such as browser, operating system, device, location, and referrer. Event names and profile data are not narrowed. |

### Reading an Export

<Note>
  The download link for a completed end-user export expires 30 days after the export finishes. Personal data export requests from Mixpanel (those submitted from your Personal Settings > Data & Privacy in Mixpanel) expire after 14 days. Submit a new request if you need the data after that.
</Note>
