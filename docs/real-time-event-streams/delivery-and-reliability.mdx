...
- Event timestamp
- Event type
- Full outbound payload (as rendered by your Liquid template)
- Destination HTTP response (status code, headers, body)

Use the log to debug specific failures, replay after fixing your endpoint, or feed samples into support tickets.
...
A Sync where **more than 5% of events fail over any 15-minute window** triggers an in-product alert to Project Admins. This catches partial outages and misconfigurations early, before they accumulate in the failed events log.
