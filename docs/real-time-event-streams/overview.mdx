...
<Note>
  **Private beta.** Real-Time Event Streams are currently available to a limited set of customers during Beta. If you'd like to participate, reach out to your Mixpanel account team.
</Note>
...
- **Events log** — a 72-hour record of events that could not be delivered after all retries, retained with the destination HTTP response for debugging.

## Delivery guarantees at a glance

- **At-least-once delivery.** You can include an `X-Mixpanel-Insert-Id` header so you can deduplicate on your side.
...
- **Latency.** p95 ≤ 120 seconds from ingestion to a confirmed 2xx response at your endpoint under normal load.
