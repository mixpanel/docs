...
- **Failed events log** — an aggregated roll-up of events that could not be delivered after all retries, retained for debugging. Shows the destination, status code, error details, and event count per window (no per-event entries or payloads).
