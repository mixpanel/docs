...
- Your project has access to the Real-Time Event Streams feature.
...
<Steps>
  ...
  <Step title="Save and enable">
    Mixpanel validates the template on save. Once enabled, matching events will begin streaming within seconds.
  </Step>
</Steps>
...
<Note>
  User properties are **not** currently available in the payload template. Only event properties and the top-level variables above are supported. If you need user attributes on the destination side, include them as event properties at tracking time or enrich after delivery.
</Note>
...
<Tip>
  Include `{{ insert_id }}` in your payload if you plan to deduplicate on the destination side
</Tip>
