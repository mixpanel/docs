...
| `unhealthy` | Deliveries are partially failing but the Sync has not tripped the error threshold. Investigate soon. |
| `disabled` | The Sync is manually paused by a Project Admin. Events are discarded; no backlog accumulates. |
| `error` | Persistent delivery failures. Triggered when more than 50% of events fail for more than 15 minutes. |
...
{/* PLACEHOLDER: Add exact thresholds for the `unhealthy` state once finalized — e.g., failure rate and time window. */}
