...
| `error` | The Sync was paused after a terminal failure (auth error, or non-auth 4xx after retries). Streaming stops until a Project Admin re-enables the Sync. |

<Warning>
  When a Sync is `disabled`, events matching the filter are **not streamed**. Mixpanel does not accumulate a backlog to replay when the Sync is re-enabled.
</Warning>
