...
| Attribute | Type | Description |
| :-- | :-- | :-- |
| **distinct\_id** |  | The distinct\_id of both the event and the matching user. |
| **event** |  | Event object. For users with no events, this property is undefined |
| **user** |  | User object with distinct\_id matching the event. For events without a distinct ID or when there's no matching user record, this property is undefined. |
...
| **global** |  | Limit the maximum number of results to display. |
