---
title: "Flows"
description: "See frequent paths taken by users to or from any event"
sidebarTitle: Flows
---
...
Turn on Collapse Repeated to combine consecutive occurrences of the same event into a single step. For example, if a user performs Search three times in a row, the flow shows one Search step rather than three. A run of any length collapses into one step. Repeats of the same event that are separated by a different event stay separate.
