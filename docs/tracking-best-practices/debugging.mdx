---
title: "Debugging"
description: "Validate your data and troubleshoot your implementation"
...
Mixpanel recommends that you create a [separate development Mixpanel project](/docs/tracking-best-practices/developer-environments#separate-development-data) to validate your event data. This ensures that your testing data does not contaminate your production environment.

### Send Events

Mixpanel doesn't receive any data until you start sending events. If you haven't started sending data to Mixpanel, check out our quickstart guides for [JavaScript](/docs/quickstart/install-mixpanel?#javascript), [Server](/docs/quickstart/install-mixpanel?#python), and [Mobile](/docs/quickstart/install-mixpanel#ios-objective-c). We have a simple [HTTP API](/docs/quickstart/install-mixpanel?sdk=http-api) for any languages we don't support.
...
<Frame>
  ![Events Filter](/images/events-filter.png)
</Frame>
...
<Frame>
  ![View User Profile in Events](/images/view-profile.png)
</Frame>
...
**⚠️ Important:** Projects with EU or India Data Residency must configure their SDK with the matching regional host (`api-eu.mixpanel.com` or `api-in.mixpanel.com`). Events sent to the wrong host will not be ingested in your Mixpanel project.
...
In addition to events and properties that haven't fired recently, Mixpanel also caches property names and property values to help dropdown menus load quickly.

If you've just implemented a new property or a new value for an existing property and don't see it appear in a report's dropdown, the cache may not have refreshed yet.

To manually refresh the dropdown cache:

1. Open the property dropdown menu
2. Type any string into the search box, e.g. "Refreshing Cache" (this doesn't need to match an actual property — it just triggers the dropdown's search state).
3. A **Refresh** option will appear in the dropdown.
4. Click **Refresh** to refresh the cache of property names and values for that dropdown.

Your new property or value should now appear without having to wait for it to automatically resurface in the UI.

<Frame>
  ![Image](/images/dropdown-refresh-button.png)
</Frame>
...
Client-Side Tracking can be unreliable, you may lose events for 30-50% of your users. You can resolve this by [sending events through a proxy](/docs/tracking-methods/sdks/javascript#tracking-via-proxy), but it requires a bit more effort. We recommend [server-side tracking](/docs/tracking-best-practices/server-side-best-practices), since it is more reliable and easier to maintain than web/mobile tracking.
