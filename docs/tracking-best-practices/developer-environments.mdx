...
**Note** An organization is charged based on data across all projects. This means that if the same event is being sent to multiple projects, it will count against your data allowance once per each project.
...
- Ensure you’re handling user identity properly so that you can track users across platforms without counting them as a different user on each platform. Learn more about [identifying users](/docs/tracking-methods/id-management/identifying-users-simplified).
...
The optional library\_name parameter in the mixpanel.init function allows you to specify which library to write to in all tracking calls.
