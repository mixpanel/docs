...
Autocapture is also available on mobile via the [Android](/docs/tracking-methods/sdks/android#autocapture) (v8.10.0 or later), [iOS](/docs/tracking-methods/sdks/swift#autocapture) (v6.6.0 or later), [React Native](/docs/tracking-methods/sdks/react-native#autocapture) (v3.7.0 or later), and [Flutter](/docs/tracking-methods/sdks/flutter#autocapture) SDKs. These SDKs capture screen views, clicks, dead clicks, and rage clicks (only screen views are currently available for Flutter — clicks, dead clicks, and rage clicks are coming soon). Click events are in beta and are turned off by default — see each SDK’s Autocapture section to set them up.
...
By default, Mixpanel considers a click on an interactive element to be a dead click if no responses are within 0.5 seconds. You can configure these qualifying criteria to something that best suits your application by overriding the defaults in the [Javascript SDK](/docs/tracking-methods/sdks/javascript).
...
Customers adding the JavaScript SDK with a snippet that contains an enabled autocapture config will turn on autocapture. **If this config is not present in the snippet, Autocapture will not be enabled.** Our documentation on how to configure the JavaScript SDK, including changing your Autocapture configuration, can be found [here](/docs/tracking-methods/sdks/javascript).

### Disabling Autocapture

You can disable Autocapture by setting the config in the JavaScript SDK. Disabling Autocapture does not disable session recording, which is [configured separately](/docs/tracking-methods/sdks/javascript#session-replay) in the JavaScript SDK. Learn more about detailed configuration [here](/docs/tracking-methods/sdks/javascript).

### Manage your event volume

Mixpanel recommends monitoring your event volume (in Organization Settings \> Plan Details & Billing) as you make changes to ensure it matches your expectations.
...
- Sensitive elements — such as end-user text inputs, selects, and textarea elements - are default-excluded from tracking. You can view our default configurations and options [here](/docs/tracking-methods/sdks/javascript).
...
- Mixpanel also provides flexibility to define the class pages of your website for which you configure Autocapture by using `block_selectors` and `block_url_regexes`.

To change these default settings, you can customize what is collected through your [SDK configuration](/docs/tracking-methods/sdks/javascript).
...
Alternatively, you can update your Autocapture configuration using different blocking options, including blocking tracking of certain classes or certain URLs. You can read more about this [here](/docs/tracking-methods/sdks/javascript).
