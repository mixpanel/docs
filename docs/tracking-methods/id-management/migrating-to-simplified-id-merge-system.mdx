...
- In March 2023, we released **Simplified ID Merge** to remove the complexities of having to rely on different [identity methods](/reference/create-identity) (i.e. \$identify, \$create_alias, \$merge) for different merging scenarios. This also removed the need to cap Distinct IDs at 500 in an ID cluster.
