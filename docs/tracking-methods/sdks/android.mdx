...
<Note>
  Autocaptured click events require Android SDK v8.10.0 or later. They are **disabled by default** — you opt in with `AutocaptureOptions` at initialization.
</Note>
...
A `$mp_dead_click` event is emitted when a tap on an **interactive** element produces no visible response. The SDK snapshots the UI immediately before the click handler runs, watches for layout changes, scrolls and new windows, and — if nothing has changed 500 ms later — emits the event. Any detected change cancels it. A dead click is always accompanied by the `$mp_click` for the same tap.
...
| `RageClickOptions` | `timeWindowMs` — window they must fall in | `1000` |
| `RageClickOptions` | `radius` — spatial threshold, in dp | `44` |
| `DeadClickOptions` | `enabled` | `true` |
| `DeadClickOptions` | `timeWindowMs` — wait before declaring a click dead | `500` |
...
val autocaptureOptions = AutocaptureOptions.Builder()
    ...
    .deadClickOptions(
        DeadClickOptions.Builder().enabled(false).build()
    )
    .build()
```

Autocapture respects opt-out: it does not start when tracking is opted out, and stops and restarts with `optOutTracking()` and `optInTracking()`.
...
<Note>
  Autocapture does not capture element text. There is no `$el_text` property, and no `$attr-aria-label`: `contentDescription` is user-facing text that can carry personal data, so it is neither reported nor used as an identifier.
</Note>
...
**XML views** (and anything React Native renders, since React Native draws through the View hierarchy):

1. **React Native `nativeID`** — the JS-side prop, read dynamically
2. **Android resource entry name** — `@+id/checkout_button` resolves to `checkout_button`
3. **`<SimpleClassName>_<hash>`** — the anonymous fallback
...
`contentDescription` is deliberately not a source on either path. It is localized, so the same element would report a different ID per language, and it can carry user data.

A generated ID — `setId(10001)` in code, rather than an `@+id` resource — has no entry name, so it falls through to the hash.
...
- **Put a stable resource ID on every element you care about.** `android:id="@+id/checkout_button"` in XML, `Modifier.testTag("checkout_button")` in Compose. Anything without one falls back to a positional hash, which is stable but not readable.
- **Put the ID on the element that handles the tap**, not on a wrapper around it. An ID on a non-interactive ancestor is never used — the walk-up looks for interactivity, then reads identity from what it finds.
- **Keep IDs static.** Never build an ID from a row index, a timestamp, a product name or any other per-render value: each variant becomes its own `$el_id` and the element can no longer be grouped in reports.
- **Don't reach for `contentDescription` as a tracking hook.** It is never read as an identifier. Set it for accessibility, and set a resource ID or `testTag` for tracking.
- **Name IDs for what the element does**, not for how it looks — `checkout_button` rather than `green_button_2`, so the name survives a redesign.
- **Roll out gradually.** Enable autocapture alongside your existing manual `track()` calls, compare volumes in the dashboard, then retire the manual calls that have become redundant. Keep manual tracking for business-specific events.
