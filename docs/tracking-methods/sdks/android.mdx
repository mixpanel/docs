...
Unlike the Swift SDK — where `trackAutomaticEvents` is an optional parameter that defaults to `false` — the Android SDK requires `trackAutomaticEvents` as an **explicit, required parameter** on every [`MixpanelAPI.getInstance()`](https://github.com/mixpanel/mixpanel-android) overload. There is no passive default at the call site; you must pass `true` or `false` when you initialize.

```java
// Automatic events disabled (recommended)
MixpanelAPI mixpanel = MixpanelAPI.getInstance(context, "YOUR_PROJECT_TOKEN", false);
...
| --- | --- | --- |
| \$ae\_first\_open | First App Open | Tracks the first time the user has opened the app. This event is retriggered if the user reinstalls the app or clears local storage. |
...
The set of automatic events is **not identical** across platforms:

- `$ae_crashed` is tracked on **Android only**. The [Swift SDK](https://docs.mixpanel.com/docs/tracking-methods/sdks/swift#legacy-automatically-tracked-events) does not track app crashes automatically.
- `$ae_iap` (In-App Purchase) is tracked on **iOS only**. The Android SDK does **not** track in-app purchases automatically. If you need IAP tracking on Android, implement it manually with `track()` calls.
...
```xml
<meta-data
    android:name="com.mixpanel.android.MPConfig.MinimumSessionDuration"
    android:value="10000" />  <!-- 10 seconds -->
```
