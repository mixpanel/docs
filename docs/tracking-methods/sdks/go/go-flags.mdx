...
Minimum supported SDK version: [`v2.2.0`](https://github.com/mixpanel/mixpanel-go/releases/tag/v2.2.0)

The **Date** and **Semantic Version** [runtime property types](/docs/featureflags#data-types-for-runtime-values) are compared inside the SDK during local evaluation, so they require the version above. On an earlier version a condition using either type does not match: `GetVariant` returns the fallback variant along with an error, and the remaining rollout groups for that flag are not checked.
