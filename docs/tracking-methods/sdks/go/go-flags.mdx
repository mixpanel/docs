---
sidebarTitle: Feature Flags (Go)
title: Implement Feature Flags (Go)
description: "Configure server-side Feature Flags in Go with the Mixpanel Go SDK, using local evaluation for low latency or remote evaluation for cohort targeting"
---
...
- You have the [Mixpanel Go SDK](https://github.com/mixpanel/mixpanel-go) installed. If not, please follow [this doc](/docs/quickstart/install-mixpanel) to install the SDK. The minimum supported version is [`v2.0.0`](https://github.com/mixpanel/mixpanel-go/releases/tag/v2.0.0)
...
If any of these conditions use a **Date** or **Semantic Version** type, as described in [Data types for runtime values](/docs/featureflags#data-types-for-runtime-values), ensure your SDK version meets the minimum supported: [`v2.2.0`](https://github.com/mixpanel/mixpanel-go/releases/tag/v2.2.0).

On an earlier version `GetVariant` returns the fallback variant along with an error, and the remaining rollout groups for that flag are not checked.
