...
title: "Mixpanel SDKs: Javascript"
---

## Getting Started

Please refer to our [Quickstart Guide](/docs/quickstart/install-mixpanel#javascript).
...
You can handle the identity of a user using the `.identify()` and `.reset()` methods. Learn more about [identity management](/docs/tracking-methods/id-management) and [identifying users](/docs/tracking-methods/id-management/identifying-users-simplified).
...
<Note>
  Enterprise customers can get the same benefit without running a proxy server. [First-Party Domains](/docs/tracking-methods/first-party-domains) let you point a subdomain you own at Mixpanel, which manages the endpoint and TLS certificate for you. Continue with the steps below if you also need to serve Mixpanel's SDK assets from your own domain, which first-party domains do not cover.
</Note>
