---
sidebarTitle: Feature Flags (Javascript)
title: Implement Feature Flags (Web)
---
...
- A Mixpanel project with Feature Flags — see [plan availability](/docs/featureflags#plan-availability)
- You have the latest version of the SDK installed (minimum supported version is [`v2.71.0`](https://github.com/mixpanel/mixpanel-js/releases/tag/v2.71.0)). If not, please follow [this doc](/docs/quickstart/install-mixpanel) to install the SDK.
...
### Runtime Event and Property Evaluation

Developers can pass properties to the SDK for runtime property evaluation or runtime event evaluation.

Runtime event evaluation evaluates tracked runtime events and their properties against the configuration in the feature flag, as detailed in the [Runtime Events guide](/docs/featureflags/runtime-events).

If any of these property filters use a **Date** or **Semantic Version** condition, as described in [Data types for runtime values](/docs/featureflags#data-types-for-runtime-values), ensure your SDK version meets the minimum supported: [`v2.83.0`](https://github.com/mixpanel/mixpanel-js/releases/tag/v2.83.0).

Runtime property evaluation evaluates properties passed in the `custom_properties` node within the context against target values defined in your feature flag.

There is no minimum SDK version for using Date or Semantic Version types in runtime property evaluation, i.e. for any properties passed in the `custom_properties` node above.
...
Retrieve every flag assignment the SDK currently holds in a single call. Both methods take no arguments and have no fallback parameter — a flag the user was not assigned is simply absent from the result rather than falling back to a default value.

<Note>
  Neither method tracks an exposure event. Unlike the single-flag getters above, `get_all_variants` and `get_all_variants_sync` do **not** send `$experiment_started`, so reading every assignment will not enroll the user in an experiment. This is the main reason to reach for these methods — inspecting assignments for debugging, logging, or hydrating your own state. Record the exposure with `get_variant_value` (or another single-flag getter) at the point where the user actually sees the feature.
</Note>

Both methods return a [`Map`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Map) keyed by flag name — **not** a plain object. `Object.keys(variants)` silently returns an empty array, so iterate the `Map` instead.

**Synchronous**

`get_all_variants_sync()` returns the assignments already in memory. It returns an empty `Map` if flags have not loaded yet, or if the persisted assignments are past their TTL.

```javascript
const variants = mixpanel.flags.get_all_variants_sync();

variants.forEach((variant, flagName) => {
  console.log(flagName, variant.key, variant.value);
});

// for...of works too
for (const [flagName, variant] of variants) {
  console.log(flagName, variant.key, variant.value);
}
```
...
`get_all_variants()` waits for an in-flight flag fetch to settle and then resolves with the assignments. It always resolves and never rejects:

- Under `networkOnly`, a failed fetch resolves to an empty `Map`.
- Under `networkFirst`, a failed fetch resolves to the persisted assignments.
- Under `persistenceUntilNetworkSuccess`, valid persisted assignments resolve immediately without waiting on the network.

It resolves to an empty `Map` if feature flags were never initialized, and it does not start a fetch of its own — it only awaits one already in progress.

```javascript
const variants = await mixpanel.flags.get_all_variants();
...
Each value is a full variant object, so [`variant_source`](#variant-source) and the rest of the variant metadata are available on every entry.

<Warning>
  Both methods hand back the SDK's internal `Map` by reference, not a copy. Mutating the result corrupts the SDK's flag state — copy it first with `new Map(variants)` if you need to modify it.
</Warning>
