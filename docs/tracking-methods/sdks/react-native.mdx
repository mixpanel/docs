...
The walk-up matters more in React Native than on either platform natively, because of **view flattening**: React Native removes intermediate `<View>` wrappers from the native tree, so the platform hit-test often returns a leaf `Text` node even where you intended the tap for its `Pressable` parent. The walk-up is what puts the identity back on the pressable — and it is unconditional, so identity always comes from the clickable ancestor even when the leaf carries an identifier of its own. Put your identifiers on the pressable, not on the text inside it.
...
```tsx
<Pressable id="checkout_button" onPress={handleCheckout}>
  <Text>Checkout</Text>
</Pressable>
```
...
- **Put the `id` on the pressable.** The walk-up resolves identity from the nearest clickable ancestor, so an `id` on a non-interactive `<View>` or on the `<Text>` inside a button is never used.
...
- **Put the `id` on the pressable**, not on a wrapper or on the text inside it. The walk-up finds the nearest *clickable* ancestor and reads identity from there, so an `id` on a non-interactive `<View>` is never used — and React Native's view flattening makes it likelier that the tap resolves to a leaf in the first place.
