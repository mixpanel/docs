...
// For Expo or React Native Web, use JavaScript mode:
// const mixpanel = new Mixpanel("YOUR_PROJECT_TOKEN", true, false);
...
`getAllVariants` returns a `Map` via a callback or a Promise. If flags have not been loaded yet, it will attempt to fetch them before returning. On failure, it resolves to an empty `Map` rather than rejecting.
...
// Callback pattern
mixpanel.flags.getAllVariants((flags) => {
  flags.forEach((variant, flagName) => {
    console.log(`${flagName}: ${variant.value}`);
  });
});
...
### Why does `{ replace: true }` have no effect in native mode?

`updateContext` works in both native and JavaScript modes, but the two modes differ in how they merge context.

- **JavaScript mode** honors the `replace` option. Pass `{ replace: true }` to replace the existing context, or omit it (the default) to merge.
- **Native mode** always merges the new context with the existing context. `{ replace: true }` is accepted for API parity but is ignored on native.

If you need replace semantics on native, call `mixpanel.reset()` and re-initialize with the desired context.
...
- **Expo apps**: Use **JavaScript mode** (`new Mixpanel(token, true, false)`), which is the automatic default in Expo since native modules are not available.
