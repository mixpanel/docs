...
### Date and Semantic Version conditions

Minimum supported SDK version: [`v3.4.0`](https://github.com/mixpanel/mixpanel-ruby/releases/tag/v3.4.0)

The **Date** and **Semantic Version** [runtime property types](/docs/featureflags#data-types-for-runtime-values) are compared inside the SDK during local evaluation, so they require the version above. On an earlier version a condition using either type does not match, and the user context is not placed in that rollout group.

Remote Evaluation is unaffected, since Mixpanel performs the comparison.
