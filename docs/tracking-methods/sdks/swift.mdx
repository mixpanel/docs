...
<Note>
  Autocaptured click events require Swift SDK v6.6.0 or later. They are **disabled by default** — you opt in with `AutocaptureOptions` at initialization.
</Note>
...
When the tap lands on a non-interactive child — a `UILabel` inside a `UIButton`, say — the SDK walks up to the nearest interactive ancestor (up to 10 levels) and reports *that* element's identity. This keeps a button from being reported as its label. If no interactive ancestor is found, the tapped element's own identity is used.

#### `[BETA]` Dead Click

A `$mp_dead_click` event is emitted when a tap on an **interactive** element produces no visible response. The SDK snapshots the UI immediately after the tap, then compares it 500 ms later; if nothing has changed, the event is emitted. A dead click is always accompanied by the `$mp_click` for the same tap.
...
Autocapture respects opt-out: it does not start when tracking is opted out, and stops and restarts with `optOutTracking()` and `optInTracking()`. It is iOS-only — not available on Mac Catalyst, tvOS, watchOS or visionOS — and does not run inside app extensions.
...
<Note>
  Autocapture does not capture element text. There is no `$el_text` property, and no `$attr-aria-label`: `accessibilityLabel` is user-facing text that can carry personal data, so it is neither reported nor used as an identifier.
</Note>
...
**UIKit** (and anything React Native renders, since React Native draws through UIKit):

1. **React Native `nativeID`** — the JS-side prop, read through the Objective-C runtime
2. **`accessibilityIdentifier`** — framework-internal values such as `_private` or `AXID-1` are skipped
...
`accessibilityLabel` is deliberately not a source on either path. It is localized, so the same element would report a different ID per language, and it can carry user data.
...
- **Put a stable `accessibilityIdentifier` on every element you care about.** `button.accessibilityIdentifier = "checkout_button"` in UIKit, `.accessibilityIdentifier("checkout_button")` in SwiftUI. Anything without one falls back to a positional hash, which is stable but not readable.
- **Put the identifier on the element that handles the tap**, not on a wrapper around it. An identifier on a non-interactive ancestor is never used — the walk-up looks for interactivity, then reads identity from what it finds.
- **Keep identifiers static.** Never build one from a row index, a timestamp, a product name or any other per-render value: each variant becomes its own `$el_id` and the element can no longer be grouped in reports.
- **Don't reach for `accessibilityLabel` as a tracking hook.** It is never read as an identifier. Set it for VoiceOver, and set `accessibilityIdentifier` for tracking.
- **On SwiftUI, expect hashes below iOS 18.** If your reports need readable IDs across older versions, wrap the control in a UIKit view you can identify, or rely on manual `trackClick` for those flows.
- **Name identifiers for what the element does**, not for how it looks — `checkout_button` rather than `blue_button_2`, so the name survives a redesign.
