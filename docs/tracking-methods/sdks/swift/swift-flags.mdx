---
sidebarTitle: Feature Flags (Swift)
title: Implement Feature Flags (Swift)
description: "Configure your iOS or macOS app for Feature Flags using the Mixpanel Swift SDK"
---
...
- You have the latest version of the SDK installed (minimum supported version is [`v5.1.3`](https://github.com/mixpanel/mixpanel-swift/releases/tag/v5.1.3)). If not, please follow [this doc](/docs/quickstart/install-mixpanel) to install the SDK.
...
Initializing the SDK with feature flags enabled requires passing a `FeatureFlagOptions` configuration to `MixpanelOptions`. This enables making an outbound request to Mixpanel servers with the current user context. The server will assign the user context to a variant for each feature flag according to how they are configured in the Mixpanel UX.
...
If any of these property filters use a **Date** or **Semantic Version** condition, as described in [Data types for runtime values](/docs/featureflags#data-types-for-runtime-values), ensure your SDK version meets the minimum supported: [`6.7.0`](https://github.com/mixpanel/mixpanel-swift/releases/tag/6.7.0).
