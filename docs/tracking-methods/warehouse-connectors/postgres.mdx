...
Run the following as a superuser (on managed services such as Amazon RDS or Cloud SQL, use an admin role that has `CREATEROLE` and is a member of `table_owner_role`), replacing `your_database`, `your_schema`, and `table_owner_role` (the role that creates tables in the schema, such as your application, dbt, or ETL role):
...
If more than one role creates tables in a schema, run the `ALTER DEFAULT PRIVILEGES` statement once per creating role. Tables created by a role that isn't listed won't be readable by `mixpanel_reader` until you grant `SELECT` on them directly. To see which roles own existing tables, run `SELECT DISTINCT tableowner FROM pg_tables WHERE schemaname = 'your_schema';`.
