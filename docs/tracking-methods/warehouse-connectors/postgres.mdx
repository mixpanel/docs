...
Replace `your_database`, `your_schema`, and `table_owner_role` (the role that creates tables in the schema, such as your application, dbt, or ETL role), then run the following as a superuser.

On managed services without a true superuser, such as Amazon RDS, Cloud SQL, or Supabase, run it as your admin role after granting that role membership in every role that owns tables in the schema, for example `GRANT table_owner_role TO your_admin_role;`. The membership must inherit privileges, which is the default for a plain `GRANT`.
...
-- Grant read access to tables that table_owner_role creates in this
-- schema later, so new tables do not silently fail to sync. Default
-- privileges apply only to objects created by the role named in FOR ROLE.
...
If more than one role creates tables in a schema, run the `ALTER DEFAULT PRIVILEGES` statement once per creating role. Tables that a role not listed there creates later won't be readable by `mixpanel_reader` until you grant `SELECT` on them directly.

To see which roles own existing tables, run `SELECT DISTINCT tableowner FROM pg_tables WHERE schemaname = 'your_schema';`. The role running the block needs privileges from every owner returned. Otherwise `GRANT SELECT ON ALL TABLES` fails with `permission denied` and grants nothing.
