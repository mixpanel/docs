...
- A Snowflake [WAREHOUSE](https://docs.snowflake.com/en/sql-reference/sql/create-warehouse) Mixpanel will use to unload data. We recommend creating a dedicated warehouse for Mixpanel to avoid impacting other workloads.
  ...
  ```
- A Snowflake [STORAGE INTEGRATION](https://docs.snowflake.com/en/sql-reference/sql/create-storage-integration) Mixpanel will use to unload data. Set STORAGE\_ALLOWED\_LOCATIONS to the unique Mixpanel-managed bucket created to receive data for this connection.
  ...
  CREATE STORAGE INTEGRATION MIXPANEL_STORAGE_INTEGRATION
    ...
    ENABLED = TRUE
    STORAGE_ALLOWED_LOCATIONS = ('gcs://<mixpanel-provided-bucket-name>/');
