...
If you have multiple Mixpanel [Organizations](../../orgs-and-projects/organizations.md) with separate billing accounts, but need to share SSO settings and email domains for login, you can share those settings with an "Admin" Organization.
...
An Admin Organization is a special type of Organization that contains Single Sign-On settings and Claimed Domains. It can have Organization Members, but you should only add administrative users to the Admin Organization to manage security. The [SCIM](../single-sign-on.md#scim) process will add Users as Organization Members.
...
This configuration supports [SCIM](../single-sign-on.md#scim).
...
{% hint style="warning" %}
If you already use Azure for SSO with Mixpanel, you must [remove existing SSO configuration](../single-sign-on.md#remove-sso-configuration) before migrating, due to constraints with “Issuer URL” and “Identifier (Entity ID).” Contact Mixpanel by [opening a Mixpanel support ticket](https://mixpanel.com/get-support).
{% endhint %}
...
Follow the steps to set up SSO in the Admin Organization as you would for a regular Organization. [SSO documentation](../single-sign-on.md).
...
2. In Mixpanel, Set up SSO for the Admin Organization and configure an application in your IDP with the corresponding settings. You can refer to the [Single Sign-on Overview section](../single-sign-on.md) in our docs or to the respective IDP docs for [Azure](./azure.md) or [Okta](./okta.md)
