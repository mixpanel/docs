...
## Overview
Events are the core of [Mixpanel's Data Model](../tutorials/plan/tracking-strategy.md#the-mixpanel-data-model). All events should have an **Event Name**, a **Timestamp** of when that event has occurred, and a **[Distinct ID](../tracking-methods/id-management.md#distinct-id)** (Mixpanel's identifier for a user) to tie all events belonging to the same user. Events can optionally have a set of properties, which describes the event in more detail.
...
{% hint style="info" %}
**Note:** There are a few use cases, such as tracking [Ad Spend](../tracking-methods/integrations/ad-spend.md), in which the event should not be tied to specific users; as such, these events should be sent without a Distinct ID, but this is the exception.
{% endhint %}
For more information about events refer to the documentation on [The Mixpanel Data Model](../tutorials/plan/tracking-strategy.md#the-mixpanel-data-model).
...
* If your goal is to analyze at high-level how users traverse through different pages: instead of tracking multiple events `Home Page Viewed` and `Pricing Page Viewed`, track a `Page Viewed` event with a `Page Name` property set to **"\/home"** or **"\/pricing"**. See [Tracking Page Views](../tracking-methods/sdks/javascript.md#tracking-page-views) in our Javascript SDK documentation on how to instrument this.
...
* Mixpanel [Lexicon](../data-governance/lexicon.md) provides a means for you to change an event's or property's display name on the UI (if needed, to make it more user-friendly).
...
> **Tip:** Learn more best practices around defining your events and properties under our tutorials for [Creating a Tracking Plan](../tutorials/plan/tracking-strategy.md#tracking-plan-methodology).
...
To make things easier, you can register these properties as super properties. If you tell us just once that these properties are important, we will automatically include them with all events sent. Super properties are stored in a browser cookie, and will persist between visits to your site. Mixpanel already stores some information as super properties by default; see a full list of Mixpanel default properties [here](./property-reference/properties.md).

Learn more about registering super properties [here](../tracking-methods/sdks/javascript.md#setting-super-properties).

## FAQ

### What types of data can I send as properties?
Mixpanel accepts arbitrary JSON as properties, including strings, numbers, booleans, lists, and objects. See our [Property Reference doc](./property-reference/data-type.md) or our [API docs](https://developer.mixpanel.com/reference/import-events) for more details.
...
Each event can have up to 255 properties. Event property names/values can be at most 255 characters in length (longer names are truncated). Event property values are limited based on data type, refer to these limits under [Supported Data Types](./property-reference/data-type.md).
