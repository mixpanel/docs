...
Group keys are project-specific, and must be set up before data is sent. To administer group keys, navigate to your Project Settings. Click **+Add Group Key** under the _Group Keys_ section.

1. Navigate to your Project Settings (requires project owner or admin permissions)
2. Click **+Add Group Key** under the _Group Keys_ section
...
1. Choose which group key will be used as the B2B Company Key. This is the key for which [Company Profiles (B2B Company Analytics)](./#company-profiles) will be generated.
...
<div data-gb-custom-block data-tag="tab" data-title='Swift (iOS)'>
...
To create or update a [group profile](./#group-profiles), similarly to user profile operations, at least one (1) profile property must be set. A set operation for a group profile that does not exist would create said profile, while it would update an existing profile.
