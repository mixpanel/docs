...
This guide covers Lookup Tables and how to use them effectively. Lookup Tables are optional; if you're new to Mixpanel, we recommend starting with our guide on [Events and Properties](./events-and-properties.md).

## Overview

Lookup Tables let you enrich your Mixpanel properties ([event properties](./events-and-properties.md) or [user profile properties](./user-profiles.md)) with additional attributes or metadata about these properties. This is done by uploading a CSV containing a join key column and additional property columns, then mapping a Mixpanel property the join key.
...
We also provide an [API](https://developer.mixpanel.com/reference/replace-lookup-table) and [Warehouse Connectors](../tracking-methods/data-warehouse.md#lookup-tables) to keep Lookup Tables synced on a recurring basis.
...
In general, it is still best to have metadata that don't change over time, and you analyze often, to be tracked as event or user properties. Do refer to the FAQ section on [When shouldn't I use Lookup Tables?](./lookup-tables.md#when-shouldnt-i-use-lookup-tables)
...
{% hint style="info" %}
Lookup Tables property values do not support [Objects and List of Objects](./property-reference/data-type.md#list-property-support) data types.
{% endhint %}
...
{% step %}
## Map the Lookup Table to an event or user profile property.
...
1. [Locally map your Lookup Table to the property within the report](./lookup-tables.md#locally-within-a-report). This option allows you to have multiple local Lookup Tables across different reports, where their context and usage are only relevant to these reports.
2. Create multiple [Custom Properties](../features/custom-properties.md) that contain the property you want to map to without any functions, then map the different Lookup Tables to each custom property in Lexicon. Since the output of these Custom Properties without functions would be identical to the property itself, the mapping will also be identical and globally accessible via the Custom Properties.
...
{% stepper %}
{% step %}
## Click the "..." button next to the breakdown property from your report and choose "Map Lookup Table".

This breakdown property will be used as the ID that the Lookup Table join key will map to.

![image](/map-ephemeral-lookup-table.png "Ephemeral Lookup Tables")
{% endstep %}

{% step %}
## Select a Lookup Table to map to the breakdown property.

Alternatively, you may upload a new Lookup Table CSV for use locally in the report.
...
Lookup Tables can be replaced with a fresh copy, either via our UI, [API](https://developer.mixpanel.com/reference/replace-lookup-table), or [Warehouse Connectors](../tracking-methods/data-warehouse.md#lookup-tables).
...
These Lookup Table properties can be used like any normal property in reports as filters or breakdowns or even in [custom properties](../features/custom-properties.md).
...
* Don't use Lookup Tables when the ID is a User ID. Instead use [User Profiles](./user-profiles.md). Mixpanel is more optimized for User Profiles, so they don't have any scale limits and support more opinionated workflows in our product (like clicking into a report and seeing the list of User Profiles).
...
All users will be able to upload and map a Lookup Table to an existing property [locally from within a report](./lookup-tables.md#locally-within-a-report). Only users with "Admin" or "Owner" roles will be able to [make the mapping persistent in Lexicon globally](./lookup-tables.md#globally-using-lexicon) for other users in the project to use.
