...
| \$name, \$first_name, \$last_name | Name, First Name, Last Name | The user's / group's general name, as well as, first and last names. These are primarily useful because they are used, if available, in various Mixpanel UI / reports. For user [Cohort Syncs](/docs/cohort-sync), only `$first_name` and `$last_name` is exported. |
...
| $warehouse_table | Warehouse Table | The name of a table (or view) in the warehouse where the event came from; useful for understanding row-level lineage of any event. See [Data Warehouse](/docs/tracking-methods/warehouse-connectors). |
