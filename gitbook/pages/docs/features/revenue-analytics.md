...
Only customers using the [Warehouse Connectors](../tracking-methods/warehouse-connectors.md) get access to these features.
</details>

<details>
<summary>What should be the shape of the data to analyze these metrics?</summary>

Once you set the [Warehouse Connectors](../tracking-methods/warehouse-connectors.md), be sure that you have [Mirror Mode](../tracking-methods/warehouse-connectors.md#mirror) turned on to ensure your data is 100% in sync with the warehouse, even if the warehouse data is updated:
  ...
  For subscription-based models - Send a monthly snapshot of the revenue per user/ account as a [Historical Profile Property](../data-structure/user-profiles.md#importing-historical-profile-values). If a user/account is not active, set the revenue data to zero for the immediate month following so we can tag this as a churned user/account.
</details>
