...
Click on connect metric, and connect the metric card to the metric with data. This is a regular Mixpanel query panel. You can choose to bring in [saved metrics](./features/saved-metrics-and-behaviors.md), or create a metric. 
...
Filtering a tree is similar to filtering a board. It filters every metric in the metric tree. If you’re looking to filter only a specific metric, please use [inline filters](./reports/funnels/funnels-advanced.md#inline-filtering-pre-query).
