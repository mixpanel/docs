...
If you haven't already, we recommend starting with our [Migration Guides Overview](../migration.md) as it details the key components of migrating to Mixpanel from other analytics tools. Below we outline specific steps and considerations when migrating from Google Analytics.
...
Note that backfilling historical data can have significant impact on your billing. Refer to [this section](../pricing.md#are-monthly-events-calculated-based-on-ingestion-time-or-event-timestamp) for more details.

#### Loading historical data via Mixpanel BigQuery Warehouse Connector 

![image](../.gitbook/assets/ga4_overview.png)

At a high-level, the migration consists of 4 steps:
1. Set up a new Mixpanel project which is on [Simplified ID Merge system](../tracking-methods/id-management.md#identity-merge-apis). 
...
3. Set up [Mixpanel Warehouse Connector](../tracking-methods/warehouse-connectors.md) to initiate data sync from BigQuery to Mixpanel  

##### Pre-migration data audit
Before migrating your data to Mixpanel, you should conduct a data audit to quickly identify the key events and properties that you want to migrate over. You can learn more about the importance of pre-migration data audit [here](../migration.md#data-audit).  
...
![image](../.gitbook/assets/ga4_events.png)
...
{% hint style="info" %}
GA4 Append syncs use BigQuery's change history ([time travel](https://docs.cloud.google.com/bigquery/docs/time-travel)) to detect newly appended rows. BigQuery retains change history for up to 7 days (configurable, minimum 2 days). If the Mixpanel sync does not run within this window, it must be reset for a full re-sync. We recommend keeping the default of 7 days to allow time to correct any unexpected access issues (e.g., a credentials change).
{% endhint %}
...
![image](../.gitbook/assets/ga4_users.png)
...
![image](../.gitbook/assets/ga4_event_validation.png)
...
Mixpanel has a [Google Tag Manager (GTM) custom template](../tracking-methods/integrations/google-tag-manager.md) which can be leveraged to implement events within an hour. Simply load the template, and you can send Mixpanel the same events you’ve setup for your GA4 instance.
...
- [Events](../what-is-mixpanel.md#events) capture granular user actions, like a Pageview, Signup, or Purchase.
- [Properties](../what-is-mixpanel.md#properties) capture attributes of those events, like which page was viewed, the UTM Campaign that led to a Signup, or the Item that was purchased.

Events let you get more granular than [sessions](../features/sessions.md), so that you can deeply understand user behavior. At the same time, Mixpanel automatically constructs sessions from events, so you can answer session-level questions too.
...
#### Google Tag Manager (GTM)
Mixpanel has a [Google Tag Manager (GTM) custom template](../tracking-methods/integrations/google-tag-manager.md) which can be leveraged to implement events within an hour. The template initializes the Mixpanel JavaScript SDK with similar calls as outlined in the above section.
...
- Once exported, your data warehouse tables can be transformed and modeled into the [event format](../data-structure/events-and-properties.md) Mixpanel expects
