...
An organization is charged based on the total number of MTUs across all projects. This means that if a user performs a qualifying event in multiple projects, they are **counted once per project**. MTUs are _not_ User Profiles – the number of MTUs in your project will not match the number of [User Profiles](../../../../docs/data-structure/user-profiles/) in your project.
...
View the top events contributing towards your MTU count by creating a monthly Insights report and querying $all\_mtu\_events which filters out event names excluded from Mixpanel's MTU calculation and applying a breakdown by “Event Name” (example report)
