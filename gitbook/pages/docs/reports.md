...
Mixpanel has several types of reports: [Insights](./reports/insights.md), [Funnels](./reports/funnels/funnels-overview.md), [Flows](./reports/flows.md), [Retention](./reports/retention.md) and other advanced reports, each with their specialized use.
...
By default, the event menu shows all events, except for [hidden events](./data-governance/lexicon.md#hide-events-and-properties).
...
- Select "event properties" to show only the [Event properties](./data-structure/events-and-properties.md).
- Select "user properties" to show only the [Profile properties](./data-structure/events-and-properties.md).
...
When you are satisfied with the analysis, and decide that this is something that you are likely to revisit, Mixpanel recommends that you save the report to a [Board](./boards.md). You can do this by pressing "Save" in the top right and selecting the Board you would like to save to, or creating a new Board.
...
You can either customize the ranges by clicking "[Customize Range](./features/advanced.md#custom-buckets)".
...
4. Mixpanel up-samples the data by multiplying by the inverse of the sampling factor. This is done for [functions](./reports/insights.md) such as totals and uniques. Functions that do not scale with users (average, min, max) will not be up-sampled.
