...
{% hint style="info" %}
UI Controls is currently available for Web. Support for iOS, Android, React Native, and Flutter is coming soon.
{% endhint %}
...
- **Remote configuration enabled in your SDK**: Your SDK must have remote configuration enabled so it can receive sampling updates from the UI. See the platform-specific guides for setup instructions:
  - [JavaScript](./tracking-methods/sdks/javascript/javascript-replay.md#remote-configuration)
  - [Android](./tracking-methods/sdks/android/android-replay.md#remote-configuration)
  - [iOS](./tracking-methods/sdks/swift/swift-replay.md#remote-configuration)
...
Session Replay is particularly valuable for understanding user frustration and identifying problematic user experiences. To find relevant replays, you can focus on interesting users (power users, purchases), interesting trends (KPIs spikes or dips), or replays where certain events or sentiment signals (like [dead clicks](./tracking-methods/autocapture.md#dead-clicks)/[rage clicks](./tracking-methods/autocapture.md#rage-clicks)) occur.
...
You will not see the "View Replays" button if your organization is on an older plan. You will need to update to the latest plan to use Session Replay (see [here](./session-replay.md#plan-availability) for details).
...
| Platform | Guidance |
| --- | --- |
| Web | Your app must include the Mixpanel Javascript SDK, with Session Replay enabled. Learn more in the developer guide [here](./tracking-methods/sdks/javascript/javascript-replay.md#session-replay-with-a-cdp). |
| iOS | Works with CDPs, but you must install the Mixpanel Session Replay SDK in your app. Read more in our [iOS implementation guide](./tracking-methods/sdks/swift/swift-replay.md). |
| Android | Works with CDPs, but you must install the Mixpanel Session Replay SDK in your app. Read more in our [Android implementation guide](./tracking-methods/sdks/android/android-replay.md). |
