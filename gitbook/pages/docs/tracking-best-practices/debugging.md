...
Mixpanel recommends that you create a [separate development Mixpanel project](../best-practices/developer-environments.md#separate-development-data) to validate your event data. This ensures that your testing data does not contaminate your production environment. 

### Send Events
Mixpanel doesn't receive any data until you start sending events. If you haven't started sending data to Mixpanel, check out our quickstart guides for [JavaScript](../quickstart/connect-your-data?sdk=javascript.md), [Server](../quickstart/connect-your-data?sdk=python.md), and [Mobile](../quickstart/connect-your-data?sdk=ios.md). We have a simple [HTTP API](../quickstart/connect-your-data?sdk=httpapi.md) for any languages we don't support.
...
![Events Filter](../.gitbook/assets/events-filter.png)
...
![View User Profile in Events](../.gitbook/assets/view-profile.png)
...
![User Profile](../.gitbook/assets/user-profile.png)
...
- [JavaScript Debug Mode](../tracking-methods/sdks/javascript.md#debug-mode)
- [iOS - Objective-C Debugging and Logging](../tracking-methods/sdks/ios.md#debugging-and-logging)
- [iOS - Swift Debugging and Logging](../tracking-methods/sdks/swift.md#debugging-and-logging)
- [Android - Debugging and Logging](../tracking-methods/sdks/android.md#debugging-and-logging)
- [React Native - Debugging and Logging](../tracking-methods/sdks/react-native.md#debugging-and-logging)
- [Flutter - Debugging and Logging](../tracking-methods/sdks/flutter.md#debugging-and-logging)
...
1. On your website, [enable debug mode](../tracking-methods/sdks/javascript.md#debug-mode).
...
- If your project has [EU Data Residency](../privacy/eu-residency.md), look for a request triggered to  `api-eu.mixpanel.com/track`.
- If your project has [India Data Residency](../privacy/in-residency.md), look for a request triggered to  `api-in.mixpanel.com/track`.
6. If the request is successful, check that the "token" in the data payload matches the token in your [Project Settings](../orgs-and-projects/managing-projects.md#access-keys). From here, you can then validate that the event was directed to the right project token and using Events, and confirm that the data is arriving correctly in Mixpanel.
...
| [EU Data Residency](../privacy/eu-residency.md) | `api-eu.mixpanel.com` |
| [India Data Residency](../privacy/in-residency.md) | `api-in.mixpanel.com` |
...
Project Owner and Admin users can hide events, event properties, and user profile properties in your [Mixpanel project through Lexicon](../data-governance/lexicon.md#hide-events-and-properties). Check Lexicon to review if the event or property may be hidden.
...
Client-Side Tracking can be unreliable, you may lose events for 30-50% of your users. You can resolve this by [sending events through a proxy](../tracking-methods/sdks/javascript.md), but it requires a bit more effort. We [recommend](../debugging/overview.md#client-side-vs-server-side-tracking) server-side tracking, since it is more reliable and easier to maintain than web/mobile tracking.

### Different Timezones

Mixpanel records all events in Coordinated Universal Time (UTC) at intake. By default, Mixpanel displays events times in US Pacific Time but this is adjustable in [Project Settings](../orgs-and-projects/managing-projects.md#manage-timezones-for-projects). Navigate to your Project Settings to determine what timezone your Mixpanel events are displayed in.
...
A cohort might show more in Mixpanel than what is actually being exported to the partner. You can find out more about [troubleshooting this here](../cohort-sync/webhooks.md).
