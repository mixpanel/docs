...
- 1M events per `group identifier` per event date (still represented as distinct_id in the rest of this doc) in each of the project's [analytic groups](../data-structure/advanced/group-analytics.md).
...
You will want to fix the implementation to identify users correctly and avoid new users being impacted. You can find more information on [identifying users in this doc](../tracking-methods/id-management/identifying-users.md).
