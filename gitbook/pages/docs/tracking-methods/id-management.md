...
{% hint style="info" %}
Already familiar with the concept of identity management? Click [here](./id-management/identifying-users-simplified.md) for a detailed implementation guide for Mixpanel ID management.
{% endhint %}
...
Distinct ID is Mixpanel's identifier used to uniquely track a user in the system. Every event data and profile update sent to Mixpanel should have a distinct_id property associated with it (except in the case of preventing [hot shards](../debugging/distinct-id-limits.md)). The Distinct ID value tells Mixpanel which user to attach your events and profile property updates.
...
{% hint style="info" %}
Learn more about migrating to Simplified API [here](./id-management/migrating-to-simplified-id-merge-system.md).
{% endhint %}
