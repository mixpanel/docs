...
1. Connect your warehouse in **Project Settings → Warehouse Sources**.
2. Select a table or view that contains your ad metrics and choose the **Ad Spend** template.
3. Map columns for required Mixpanel fields (event name, timestamp) and ad metrics like `cost`, `clicks`, and `impressions`.
4. Set a sync schedule (e.g. hourly or daily). Mixpanel will sync rows from your table and turn each one into an event you can analyze in Insights and other reports.

## Importing via APIs

The sections below walk through building custom API pipelines to pull data directly from ad networks and send it to Mixpanel. This approach gives you full control but requires writing and hosting code.
