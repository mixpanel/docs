...
Once the library is installed, create an instance of Mixpanel in your code by calling the [`NewApiClient()`](https://pkg.go.dev/github.com/mixpanel/mixpanel-go#NewApiClient) using [your project token](../../orgs-and-projects/managing-projects.md#find-your-project-tokens).
...
{% hint style="warning" %}
For projects with EU or India data residency, you must configure the SDK to use the correct regional endpoint. Events sent to the wrong region will not be ingested. Learn more about [Privacy-Friendly Tracking](./go.md#privacy-friendly-tracking).
{% endhint %}
...
Use the [`Import()`](https://pkg.go.dev/github.com/mixpanel/mixpanel-go#ApiClient.Import) function to import events that occurred more than 5 days in the past. The `Import()` function is based on the [/import API endpoint](https://developer.mixpanel.com/reference/import-events).
