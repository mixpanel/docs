...
Please refer to our [Quickstart Guide](../../quickstart/connect-your-data?sdk=javascript.md).
...
{% hint style="warning" %}
For projects with EU or India data residency, you must configure the SDK to use the correct regional endpoint. Events sent to the wrong region will not be ingested. Learn more about [Privacy-Friendly Tracking](./javascript.md#privacy-friendly-tracking).
{% endhint %}
...
```
Note that disabling Autocapture does not disable Session Replay, which is [configured separately](./javascript.md#session-replay).
...
{% hint style="info" %}
The [/track endpoint](https://developer.mixpanel.com/reference/track-event) will only validate events with timestamps within the last 5 days of the request. Events with timestamps older than 5 days will not be ingested. See below on best practices for [historical imports](./javascript.md#importing-historical-events).
{% endhint %}
...
{% hint style="info" %}
Avoid using names containing `$` or `mp` (e.g. `$name`, `mp_country_code`, etc.) as they are used by our [reserved event properties](../../data-structure/property-reference/reserved-properties.md#reserved-event-properties) which receive special treatment in our UI or are used for special processing.
{% endhint %}
...
The default page view event has the event name (`$mp_web_page_view`), and includes the event properties page title (`current_page_title`), URL components (`current_domain`, `current_url_path`, `current_url_protocol`, `current_url_search`), and [marketing parameters](./javascript.md#tracking-utm-parameters). Additional page view event properties can be added.
...
Learn more about [UTM tracking](../../tracking-best-practices/traffic-attribution.md#web-attribution).
...
You can handle the identity of a user using the `.identify()` and `.reset()` methods. Learn more about [identity management](../id-management/identity-management.md) and [identifying users](../id-management/identifying-users.md).
...
Mixpanel [Group Analytics](../../data-structure/group-analytics.md) is a paid add-on that allows behavioral data analysis by groups (e.g. company, team), as opposed to individual users.
...
Learn more about [debugging](../../tracking-best-practices/debugging.md#debugging-with-the-browser-console-web).
...
Learn more about [EU Data Residency](../../privacy/eu-residency.md).
...
Learn more about [India Data Residency](../../privacy/in-residency.md).
...
To disable [default properties](../../data-structure/property-reference/default-properties.md) from being tracked, add them to the `property_blacklist` config flag when initializing the library.
...
{% hint style="info" %}
**Why different upstream hosts?** Most Mixpanel endpoints use `api.mixpanel.com`, but Session Replay's `/record` endpoint uses `api-js.mixpanel.com`. Your proxy must route each to the correct host or recordings will fail.
{% endhint %}
```nginx
# -------------------------------------------------------------------
...
location ~* ^/_mp/(track|engage|groups|import)(/.*)?$ {
    set $mixpanel_upstream "api.mixpanel.com";
    proxy_pass https://$mixpanel_upstream/$1$2$is_args$args;
    proxy_set_header Host $mixpanel_upstream;
...
# Session Replay → api-js.mixpanel.com
# The SDK posts gzipped application/octet-stream payloads to /record/.
# You MUST forward Authorization, Content-Type, and Content-Encoding
# headers exactly as-is or recordings will corrupt / fail.
location ~* ^/_mp/record(/.*)?$ {
    set $mixpanel_upstream "api-js.mixpanel.com";
    proxy_pass https://$mixpanel_upstream/record$1$is_args$args;
    proxy_set_header Host $mixpanel_upstream;
    proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
    proxy_set_header X-Real-IP $remote_addr;
    proxy_http_version 1.1;
...
# CDN assets (JS SDK, Session Replay recorder, targeting scripts)
# Proxying these prevents blockers from targeting cdn.mxpnl.com.
location /_mp/lib.min.js {
    set $mixpanel_cdn "cdn.mxpnl.com";
    proxy_pass https://$mixpanel_cdn/libs/mixpanel-2-latest.min.js;
    proxy_set_header Host $mixpanel_cdn;
}

location /_mp/lib.js {
    set $mixpanel_cdn "cdn.mxpnl.com";
    proxy_pass https://$mixpanel_cdn/libs/mixpanel-2-latest.js;
    proxy_set_header Host $mixpanel_cdn;
}

location /_mp/recorder.min.js {
    set $mixpanel_cdn "cdn.mxpnl.com";
    proxy_pass https://$mixpanel_cdn/libs/mixpanel-recorder.min.js;
    proxy_set_header Host $mixpanel_cdn;
}

location /_mp/targeting.min.js {
    set $mixpanel_cdn "cdn.mxpnl.com";
    proxy_pass https://$mixpanel_cdn/libs/mixpanel-targeting.min.js;
    proxy_set_header Host $mixpanel_cdn;
}
```
...
Define your proxy host and set `MIXPANEL_CUSTOM_LIB_URL` **before** the Mixpanel JS snippet (not required if you load the SDK via npm/yarn):
```javascript
const MIXPANEL_PROXY_HOST = "https://your-domain.com/_mp";
const MIXPANEL_CUSTOM_LIB_URL = MIXPANEL_PROXY_HOST + "/lib.min.js";
```

Then pass the proxy host and CDN asset paths in `mixpanel.init`:
```javascript
mixpanel.init("YOUR_PROJECT_TOKEN", {
    api_host: MIXPANEL_PROXY_HOST,
    recorder_src: MIXPANEL_PROXY_HOST + "/recorder.min.js",
    targeting_src: MIXPANEL_PROXY_HOST + "/targeting.min.js",
});
```
...
| `recorder_src` | Loads the Session Replay recorder script from your proxy instead of `cdn.mxpnl.com` |
| `targeting_src` | Loads the targeting script from your proxy instead of `cdn.mxpnl.com` |

{% hint style="warning" %}
If you use Session Replay but only set `api_host` without proxying the CDN assets, ad-blockers may still block the recorder and targeting scripts loaded from `cdn.mxpnl.com`.
{% endhint %}
...
|---|---|
| Session Replay recordings missing or corrupted | `/record` not routed to `api-js.mixpanel.com`, or `Authorization` / `Content-Encoding` headers stripped by the proxy |
| CORS errors on `/record` | Proxy is rewriting the `Host` header or not forwarding it correctly |
| 4xx/5xx on tracking calls | Proxy path prefix not stripped properly — verify `proxy_pass` rewrites match your location blocks |
| Ad-blocker still blocks requests | CDN assets (`lib.min.js`, `recorder.min.js`, `targeting.min.js`) are still loaded directly from `cdn.mxpnl.com` — set `MIXPANEL_CUSTOM_LIB_URL`, `recorder_src`, and `targeting_src` |

## Session Replay

See the developer's guide to implementing Session Replay with the Javascript SDK [here](./javascript/javascript-replay.md).
