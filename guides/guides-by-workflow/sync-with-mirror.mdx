---
sidebarTitle: Sync with Mirror
title: Eliminate Data Drift Between Your Warehouse and Mixpanel Using Mirror
description: "Keep Mixpanel in sync with your warehouse using the Mirror connector"
---

When your warehouse is the source of truth, you need Mixpanel to stay in sync with it — not just ingest a snapshot and drift. [Mirror sync mode](/docs/tracking-methods/warehouse-connectors#mirror) does that by tracking every change in your warehouse (new rows, updates, and deletions) and reflecting them in Mixpanel automatically. The result is a Mixpanel project that always matches your warehouse, without manual reconciliation.
...
- Access to a [supported warehouse](/docs/tracking-methods/warehouse-connectors#step-1-connect-a-warehouse), such as Snowflake, BigQuery, Databricks, or Redshift
...
Each event supports a maximum of 255 properties. Property names longer than 255 characters are truncated. For [data type](/docs/data-structure/property-reference/data-type) details and a full list of [events and properties limits](/docs/data-structure/events-and-properties#what-are-the-limits-of-events-and-properties), refer to the Mixpanel docs.
...
[User Profiles](/docs/data-structure/user-profiles) and [Group Profiles](/docs/data-structure/group-analytics#group-profiles) capture the current state of a person or group. They're mutable by design — you can update them any time, and they join retroactively with all past events sharing the same identifier. That means you don't need to track events and profiles in lockstep; Mixpanel applies the current state at query time.
...
The [`distinct_id`](/docs/tracking-methods/id-management) is the primary join key in Mixpanel. It connects events to user profiles and drives all core analyses — Funnels, Retention, and unique user counts.
...
<Note>
  **Pro tip**: Unlike client-side SDK tracking, Mirror doesn't handle ID bridging automatically. [Validate your identity mapping](/docs/tracking-methods/id-management) before your first sync, not after. A mismatch across millions of rows is expensive to fix.
</Note>
...
- **Event volume alerts**: Set automated alerts in [Data Volume Monitoring](/docs/data-governance/data-volume-monitoring) to catch unexpected spikes. These often signal an upstream warehouse issue or an unplanned schema change triggering a resync.
...
👉 **Next step**: For guidance on naming conventions, metadata, and access controls once your sync is live, see [Govern Your Mixpanel Data for Long-Term Success](/guides/guides-by-topic/govern-data).
