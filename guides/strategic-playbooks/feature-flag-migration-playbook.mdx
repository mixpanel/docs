...
| :-- | :-- | :-- | :-- | :-- |
| Experiment | statsig::experiment\_exposure | in Streaming, feature event where reason.inExperiment = true<br />in Warehouse, evaluation\_events row where experiment\_iteration\_id IS NOT NULL | campaign\_activated event/decisions dataset | Experiment Viewed |
| Feature Gate | statsig::gate\_exposure | in Streaming, feature event where reason.inExperiment is absent/false<br />in Warehouse, evaluation\_events row where experiment\_iteration\_id IS NULL (boolean flag) | Feature Rollout | — (no gate exposure concept) |
| Dynamic Configuration | statsig::config\_exposure | in Streaming, feature event where reason.inExperiment is absent/false<br />in Warehouse, evaluation\_events row where experiment\_iteration\_id IS NULL (non-boolean flag) | Flag with Variables | — (no config exposure concept) |
...
```javascript
{
	...
	"properties": {
		...
		"$insert_id": "exp_4821_alice_1709275888", // for deduplication
...
Your vendor may have a UI export, API export, or warehouse export option. Ensure you list and fetch definitions for feature gates, experiments, and dynamic configurations. 
...
```javascript
{
  ...
  "workspace_id": null,
  "is_experiment_active": null, // this will set the experiment to disabled in the UI. Upon confirming configuration, you can re-enable it
  "hash_salt": null,
...
Go to 'Feature Flags' and search for the name of the feature flag that you have imported via the Mixpanel API. 
...
Once you have imported the experiment into Mixpanel, a corresponding Mixpanel Experiment report will be created. You will need to continue populating this experiment with the right  configuration and metrics. Go to Experiments, and search for your experiment by name.

<Frame>
  ![Screenshot 2026 07 21 At 4 36 21 PM](/images/Screenshot-2026-07-21-at-4.36.21-PM.png "Screenshot 2026 07 21 At 4 36 21 PM")
</Frame>
