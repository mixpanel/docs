...
sidebarTitle: "Feature Flag Migration Playbook"
---
...
By the end, your flags and experiments will be set up in Mixpanel, your historical results will carry over, and your application code will be evaluating flags through Mixpanel rather than your old provider.

<Note>
  This guide is a reusable template. The steps below are vendor-agnostic; the worked examples use Statsig. To migrate from a different provider (LaunchDarkly, Optimizely, Firebase Remote Config, VWO, GrowthBook, or any OpenFeature setup), substitute that vendor's export API in each "Export" step — the Mixpanel side stays the same.
</Note>
...
2. **Migrate flag, experiment, and dynamic config configurations** (both the setup and the reporting).
...
An exposure event records that a user was enrolled in an experiment, and was bucketed into an experiment variant. To keep your historical data, export your existing exposure events from your current vendor and ingest them to Mixpanel as `$experiment_started` events. We recommend running the migration only for completed experiments.
...
You can export them in a few ways:

- **Programmatic**: Use the Statsig Console API to pull the data.
...
| Mixpanel | Statsig | LaunchDarkly | Optimizely | GrowthBook |
| :-- | :-- | :-- | :-- | :-- |
| Experiment | statsig::experiment\_exposure | feature event w/ inExperiment=true (streaming); evaluation\_events rows w/ experiment\_iteration\_id (warehouse) | campaign\_activated event / decisions dataset | Experiment Viewed |
| Feature Gate | statsig::gate\_exposure | feature event (boolean flag eval) | — (no decision event for deliveries/rollouts) | — (no gate exposure concept) |
| Dynamic Configuration | statsig::config\_exposure | feature event (multivariate/JSON flag eval) | — (variable values returned w/o decision event) | — (no config exposure concept) |
...
```javascript
{
	...
	"properties": {
		...
		"time": 1709275888,
		"$insert_id": "statsig_exp_4821_alice_1709275888",
...
In **Statsig**, call the Console API (base [https://statsigapi.net/console/v1](https://statsigapi.net/console/v1), header STATSIG-API-KEY) to list and fetch definitions:

- `GET /gates` and `GET /gates/{id}` — feature gates (boolean flags), with their rules and rollout
- `GET /dynamic_configs` — dynamic configs (multi-value configurations)
- `GET /experiments` and `GET /experiments/{id}` — experiments, including groups/variants, allocation %, targeting, and the unit ID type

For other vendors, substitute the equivalent read endpoints (LaunchDarkly flags API, Optimizely flags API, etc.).

### Import the setup into Mixpanel

Now that you've brought over your historical exposure data, it's time to bring over the configurations themselves.

Recreate each configuration in Mixpanel using Mixpanel APIs. Create experiments first, then flags that reference them.
...
```javascript
{
  ...
  "description": "Migrated from Statsig experiment: checkout_button_test",
  "hypothesis": "Larger, more prominent checkout buttons will increase conversion rate",
  "variants": [
    {
      "name": "control",
      "description": "Original Statsig group: Large Button"
    },
    {
      "name": "treatment",
      "description": "Original Statsig group: Green Button"
    }
  ],
  "metrics": [],
  "settings": {
...
- **Create Feature Flag**: POST [`/api/app/projects/{project_id}/workspaces/{workspace_id}/feature-flags`](https://docs.mixpanel.com/reference/create-feature-flag-1).
  - For a Feature Gate, the key is to specify the control and treatment values in the `variants.value` object as a boolean value.
    ...
    "variants": [
      {
        ...
        "is_sticky": false,
        "split": 0.5,
        "description": "Control — Migrated from Statsig"
      },
      {
        "key": "treatment",
        ...
        "is_sticky": false,
        "split": 0.5,
        "description": "Treatment — Migrated from Statsig. Original: pass"
      }
    ],
    "rollout": [
      {
...
```javascript
// the following is for Node.js SDK experiments implementation
