...
An exposure event records that a user was enrolled in a feature gate, an experiment, or a dynamic config, and was bucketed into a variant. To keep your historical data, export your existing exposure events from your current vendor and ingest them to Mixpanel as `$experiment_started` events.  An exposure event records that a user was enrolled in a feature gate, an experiment, or a dynamic config, and was exposed to a specific variant. To keep your historical data, export your existing exposure events from your current vendor and ingest them to Mixpanel as `$experiment_started` events.
...
- The assigned variant
 ...
 Details can be found at  [https://docs.mixpanel.com/docs/experiments#implementation-for-experimentation](https://docs.mixpanel.com/docs/experiments#implementation-for-experimentation).  Details about Mixpanel's `$experiment_started` event can be found [here](/docs/experiments#implementation-for-experimentation).
...
| Feature Gate | `statsig::gate_exposure` | **_<u>Streaming</u>_** <br />`Feature` event where `reason.inExperiment` is absent/false<br /><br />**_<u>Warehouse</u>_** <br />`evaluation_events` where `experiment_iteration_id` is absent | Feature Rollout | (no gate exposure concept) |
| Dynamic Configuration | `statsig::config_exposure` | **_<u>Streaming</u>_** <br />`Feature` event where `reason.inExperiment` is absent/false<br /><br />**_<u>Warehouse</u>_** <br />`evaluation_events` where `experiment_iteration_id` is absent | Flag with Variables | (no config exposure concept) |
...
Your vendor may have a UI export, API export, or warehouse export option. Ensure you list and fetch definitions for feature gates, experiments, and dynamic configurations.  Your vendor may have a UI export, API export, or warehouse export options. Ensure you list and fetch definitions for feature gates, experiments, and dynamic configurations by consulting your vendor's documentation.

### Import the setup via Mixpanel API 

Recreate each configuration in Mixpanel using Mixpanel APIs. Create experiments first, then flags that reference them. You can refer to the Mixpanel Feature Flags API for more details: [https://docs.mixpanel.com/reference/feature-flags-api](https://docs.mixpanel.com/reference/feature-flags-api)  Recreate each configuration in Mixpanel using Mixpanel APIs. Create experiments first, then the feature flags that reference them. You can refer to the [Mixpanel Feature Flags](/reference/feature-flags-api) API for more details.
...
If you are currently using LLMs, we also recommend exploring this migration approach with the Mixpanel MCP server: [https://docs.mixpanel.com/docs/mcp](https://docs.mixpanel.com/docs/mcp).  The MCP server can read all the flags and experiments in your project, and will be a good assistant in making code changes.  If you are currently using LLMs, we also recommend exploring this migration approach with the [Mixpanel MCP server](/docs/mcp). The MCP server can read all the flags and experiments in your project, and will be a good assistant in making code changes.
