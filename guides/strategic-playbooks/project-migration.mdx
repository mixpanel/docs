---
title: Project Migration Playbook
description: "Consolidate siloed Mixpanel projects and migrate data during company growth"
---
...
<Frame>
  ![image](/images/simplified_api.png)
</Frame>
...
- If you haven’t done so already, consider running an [analytics strategy session](/guides/strategic-playbooks/product-analytics-strategy/get-started) to develop a framework of KPIs and identify the events and properties that are needed to unlock this analysis.
...
Once you have determined how far back your data requirement is you can export this data via our [Raw Export API](/reference/raw-event-export) or, if your organization has our Data Pipelines add-on, you can leverage the data you already have in your data warehouse. We recommend exporting a raw pipeline, as opposed to a schematized pipeline, this will export events as JSON to a cloud storage bucket.

With the [raw data export API](/reference/raw-event-export) you can export all event data as the 'event' parameter is optional. See example request below:
...
If you are not leveraging our warehouse connectors offering, you can use our [import API](/reference/import-events) instead. Review the GCS batch import script below and modify it to suite your needs (i.e. If your project is stored in the EU adjust your API endpoints accordingly):
...
If you have not purchased warehouse connectors, you can leverage our [Engage API](/reference/profile-set) instead. Here's some sample code to get you started, utilizing the `$set` operator to update user profiles:
...
- **Boards & Reports**: Utilize [Move](/docs/boards/move-boards) to transfer saved boards and reports across the same region (i.e. US, EU data centers). Permissions are managed by group admins allowing Boards to be moved across Projects or Organizations, depending on your use case.
  ...
  - Lexicon metadata can be [retrieved](/reference/list-all-schemas-for-project) from existing projects and [recreated](/reference/upload-schemas-for-project) in the new project via our Lexicon Schemas API.
