...
Your AI tool's connector permissions control whether the connection is scoped to read-only, so it can only answer questions and never change anything in Mixpanel. That's separate from your existing Mixpanel role: You'll only ever see the projects and data you already have permission to view in Mixpanel, regardless of how the connector is scoped.
...
[RCA Agent](/guides/mixpanel-agent#rca-agent) surfaces a confidence level for each finding, signaling how much to trust a given finding. You can also rate the RCA board's accuracy yourself after reviewing it, which improves future results.
...
- Your AI tool's connector permissions control whether the connection exposes write operations, while your existing Mixpanel roles and project permissions continue to determine which operations are allowed.
...
Before utilizing Mixpanel’s AI tools, write [Business Context](/guides/use-mixpanel-ai/write-business-context). Mixpanel’s AI tools read it automatically, so it improves every answer that follows, not just the one in front of you.