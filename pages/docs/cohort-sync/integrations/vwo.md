...
In addition, when our ingestion service detects calls setting this user property, Mixpanel will also auto-alias the value of `$vwo_user_id` to the user's distinct_id. This ensures that experiment data passed from Leanplum to Mixpanel still attributes to the correct user.
