...
- Only Owners have permission to creat drop filters and drop data.
...
<Callout type="warning">
    Dropped events are not stored and cannot be recovered. Review your filter carefully before saving. Filters are active 10 minutes after creation and while drop filters can be deleted, the data dropped while they were active cannot be recovered unless sent to Mixpanel again.
</Callout>

![image](/drop-filter-1.png)
