...
<Callout type="info">
 Note that events merged into another will no longer be available in reports or custom events, so be sure to update these with the new merged event name.
</Callout>
...
The payload returned from the Lexicon Schemas API is most likely not an exact match of the Lexicon as seen in your project's UI. This because the Lexicon Schemas are a subset of the data that appears in Lexicon UI.

The Lexicon UI contain entities that were sent to your project within the last 30 days or has an associated Lexicon Schema.

A Lexicon Schema is created when one of the following take place:
...
When using the Lexicon Schema API to export, only entities that has a Schema are exported. Entities that are seen in your Lexicon UI that were sent within the last 30 days without a Schema will not be reflected in the Lexicon Schema payload.
