...
<Callout type="info">
  You need `CREATE CATALOG` privilege to create a new catalog:
  1. Go to Catalog → settings (gear icon) → metastore
  2. Click "Grant" button in "Permissions" tab and grant "CREATE CATALOG" to the user
</Callout>
<Callout type="info">
  If you are using an existing shared catalog, ensure you have the following privileges on that catalog:
  ```sql
