...
In this guide, we walk through how to set up Data Pipelines by following these steps:
1. Configure your destination to allow Mixpanel to write to it.
2. Create the Pipeline to start exporting your data to that destination.
3. Query the exported data in the destination service.

We offer a 30-day free trial of the Data Pipelines. See the [FAQ](#how-does-the-free-trial-work) for how to enable it.


## Step 1: Configure the destination

There are two types of Pipelines: Raw & Schematized. Each is configured slightly differently.

### Raw

Raw Pipelines export events as JSON to a cloud storage bucket. 
...
- Hourly: `<BUCKET>/<PREFIX>/<PROJECT_ID>/<YEAR>/<MONTH>/<DAY>/<HOUR>`
- Daily:  `<BUCKET>/<PREFIX>/<PROJECT_ID>/<YEAR>/<MONTH>/<DAY>/full_day`
...
Schematized Pipelines export events into tables with schemas generated by Mixpanel. There are two types of schemas, which you can configure:
...
The [Schematized Pipeline reference](/docs/data-pipelines/schematized-export-pipeline) goes into the details of schematization and the output format.

## Step 2: Create the Pipeline

Once you’ve configured your destination, you need to tell Mixpanel to start exporting to that destination. You can do this with our [Create Pipeline API](https://developer.mixpanel.com/reference/create-warehouse-pipeline).
...
### What are the limits?
* For event export pipelines (`data_source: events`), we support at most two recurring pipelines (`to_date` is empty) and one non-recurring pipeline (has a `to_date` that is the ending date of the export window).
* We support backfilling at most 6 months of data. This means `from_date` must also be no more than 6 months from the date the pipeline is created.
