...
Customers on the Growth plan can buy this add-on [online](https://mixpanel.com/pricing/), while customers on the Enterprise plans can reach out to their Customer Success Manager or the [Mixpanel Sales Team](https://mixpanel.com/contact-us/sales/). Online plans allow up to 6 group keys. Enterprise customers can choose between 3 or 6 group keys. Each group key can have a total of 1M group profiles (see [Limits and Caveats](https://docs.mixpanel.com/docs/data-structure/advanced/group-analytics#limits-and-caveats)).
...
### Limits and Caveats
The maximum number of group profiles that may be tracked is 1M profiles per group key. When exceeding this limit, queries that use group profile properties against user level analysis (eg: break down my user funnel by company group key) may fail due to the size of the group table.
