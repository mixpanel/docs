...
Customers on the Growth plan can buy this add-on [online](https://mixpanel.com/pricing/), while customers on the Enterprise plans can reach out to their Customer Success Manager or the [Mixpanel Sales Team](https://mixpanel.com/contact-us/sales/). Online plans allow up to 6 group keys. Enterprise customers can choose between 3 or 6 group keys. Each group key can have a total of 1M group profiles (see [Limits and Caveats](/docs/data-structure/advanced/group-analytics#limits-and-caveats)).
...
Below set limits ensure reports using group analytics perform correctly, especially when they are analyzed against user level data (e.g. break down my user funnel by company group key):

- 1M group profiles per group key (e.g. companny_id)
- 1M events per day per group identifier (e.g. company_id = "Mixpanel") 
...
[Session events](/docs/features/sessions) are not supported when analyzed by groups in a report.
...
Alternatively, the `<data_group_id>` can also be seen as part of the URL of the [Group Profile](/docs/data-structure/advanced/group-analytics#group-profiles) page:
