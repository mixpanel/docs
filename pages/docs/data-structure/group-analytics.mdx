...
This page gives you a view into all your company health - this includes key metrics and company profile properties (i.e company attributes).
...
2. This is only available if you have [set up a B2B Company Key](/docs/data-structure/group-analytics#setup-b2b-company-key), an option available in the Group Analytics package.

![image](/B2B_Company_Profiles.webp) 

#### Activation Metrics 

SaaS companies often have use cases like the one below, where you’re trying to segment companies based on the number of users and the quality of users. (Examples - what is the number of trial accounts that have > 2 active users?)  The hypothesis is that these will convert faster. What you want to do here is to be able to break down account activity by the number of active users. We have a computed property in both breakdown and filter, called the **“Number of users who did…”**, available in the Group Analytics add-on, which enables you to answer these types of questions.
...
1. Choose which group key should be used as the B2B Company Key as well. This is the key for which Company Profiles will be generated.
