...
Lookup Tables are accessible via Lexicon. Go to **Lexicon > Import > Lookup Table** to upload a CSV file or manage existing lookup tables. Only project Owners and Admins can create global Lookup Table mapping via the Lexicon. Consumers and Analysts can only [create local mappings within reports](/docs/data-structure/lookup-tables#locally-within-a-report).
