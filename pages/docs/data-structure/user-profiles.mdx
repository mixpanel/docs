...
Historic updates for profile property values layer on additional capabilities by capturing changes in each property over time instead of just the latest value. At this time, this functionality is dependent on our wahourse connectors feature. You can read more on how to import this type of data [here](../tracking-methods/warehouse-connectors#profile-history-tables).
...
Each entry in a Profile History table generates an event; updating the value on that row or deleting the row will thus generate an update or deletion. For event plans, the event tally will also add the number of any event, update or delete from warehouse connectors, including profile history tables. This differs from basic profile tables (which only update the current value, there's no history), which leads to profile tables not counting towards billing but profile history ones do.

For MTU plans, the events and updates/deletes don't directly affect the MTU tally unless the volume of events and updates pushes the threshold for the guardrail (by default 1000 events per user); once the throshold for the guardrail is crossed, events and updates from profile history tables will also count towards billing.
