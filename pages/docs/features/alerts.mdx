...
Suggested page structure (in order):
  ...
  9. Root Cause Analysis             [UNCHANGED]
=============================================================================
*/}
...
<Callout type="info">
    To create and manage alerts, you will need an analyst, admin, or owner project role. Learn more about [Roles and Permissions](https://docs.mixpanel.com/docs/orgs-and-projects/roles-and-permissions).
</Callout>
...
1. Navigate to **Gear Icon** > **Integrations** > **Notification Webhook**. Note this is separate from [Custom Webhooks](https://docs.mixpanel.com/docs/cohort-sync/webhooks) (which are used to sync cohorts).
...
{/* ============================================================ */}
{/* SECTION 9: ROOT CAUSE ANALYSIS — [UNCHANGED] */}
{/* No content changes. Copy as-is from live doc. */}
{/* ============================================================ */}

## Root Cause Analysis

Mixpanel's Root Cause Analysis (RCA) feature analyzes the properties of unexpected events and automatically generates a Board with reports broken down by relevant properties, so you can investigate the underlying cause of any anomalous data point or triggered alert.

RCA Boards can be accessed from alert trigger messages delivered via Slack, email, or webhook. Each alert is tied to one Root Cause Analysis Board that can be accessed and edited by anyone in your organization. With this one-to-one pairing, edits made by one teammate are visible to others. Duplicate the board if you want to make independent edits.

If the original Board is deleted, another can be generated from the alert trigger message.

### Root Cause Analysis Board

The RCA Board provides a comprehensive view of the data related to the triggered alert or selected data point. Mixpanel scans the top properties of the original event and generates a report for each property, so you can visually explore the relationship between properties and the anomalous data point.

Key features:

- **12 segmented reports** are generated, showing the triggered event broken down by its top relevant properties.
- Each segmented report is **filtered by a date range** that matches the original alert's time window — using a custom date range (not a relative one) to ensure consistency on future visits.
- The board includes **links to the original report** where the alert was set up, as well as a link to the alert's configuration settings.

By leveraging Root Cause Analysis, you can quickly identify the factors driving anomalies, take targeted action, and collaborate with your team without losing context.
