...
Tag teammates in a comment by typing `@` followed by their name. Mentioned users will receive a notification in the notification center and, if configured, via email, slack, or teams.
...
On reports, the report owner can enable or disable comments using the toggle in the report settings. When comments are disabled, existing comments are preserved but hidden and no new comments can be added.

Comments on boards are always enabled for users with the appropriate permissions.
