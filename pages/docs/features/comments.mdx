...
{<h3>Reply to an existing thread.</h3>}
...
Tag teammates in a comment by typing `@` followed by their name or clicking the `@` symbol underneath the composer box. Mentioned users will receive a notification in the notification center and, if configured, via email, slack, or teams.
