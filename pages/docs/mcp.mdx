...
- Go to **Settings → Tools & MCP → New MCP Server** to open `~/.cursor/mcp.json`, then add:
- Once Cursor connects, it will prompt you to authorize with Mixpanel.
...
Or edit `~/.gemini/settings.json` and add:
...
Any client that supports the MCP JSON config format can connect using the same snippet above. The first time it connects it will prompt you to authorize with Mixpanel.
...
- Once enabled, any Mixpanel user in your organization can connect. No additional configuration needed. 
- Users authenticate with their own Mixpanel credentials, so all existing project permissions and roles apply. 
- Users can only see data from projects they already have access to.
