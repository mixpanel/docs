...
<Callout type="warning">
  **Experiments and Feature Flags tools are in beta.** They require opt-in configuration. See [Enabling Beta Features](#beta-experiments--feature-flags) below. Audience targeting and cohort editing are not yet supported.
</Callout>
...
## Beta: Experiments & Feature Flags

<Callout type="warning">
  Experiments and Feature Flags tools are in **beta**. To enable them, you need to pass an additional header when connecting to the MCP server. Instructions vary by client — see below. After updating your configuration, restart your client to load the new tools.
</Callout>

### Claude Desktop

1. Open **Settings → Developer** and click **Edit Config** to open `claude_desktop_config.json`
2. Update your Mixpanel entry to pass the feature flag header:
...
"mcpServers": {
  "mixpanel": {
    ...
    "args": [
      ...
      "--header",
      "X-Feature-Flags:experiments_and_feature_flags_tools"
    ]
  }
}
```

3. Completely quit and relaunch Claude Desktop. A browser tab will open to re-authenticate.

### Claude Code

Run the following command to add (or replace) the Mixpanel MCP server with beta tools enabled:

```bash
claude mcp add --transport http mixpanel https://mcp.mixpanel.com/mcp --header 'X-Feature-Flags: experiments_and_feature_flags_tools'
```

Then reload tools in your current session with `/mcp` or start a new session.

### Cursor

Update your `~/.cursor/mcp.json` entry to include the header argument:
...
Any client using the MCP JSON config format can enable beta tools by adding `"--header"` and `"X-Feature-Flags:experiments_and_feature_flags_tools"` to the `args` array, following the Cursor example above.
