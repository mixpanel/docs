...
1. Under "Project Settings", click **Project Users**.
2. Select the **Custom Roles** tab to see a list of current roles in the project.
3. Click **Create Role**.
...
6. Configure permissions by toggling individual permissions on or off across the available categories (see [Permission Categories](/docs/orgs-and-projects/roles-and-permissions#permission-categories) below).
7. Click **Save** to create the role.

Once created, the custom role will appear alongside default roles when assigning project roles to users.
...
|---|---|
| **Analysis** | Save reports & boards, create custom data definitions (cohorts, custom events, custom properties, metrics, behaviors), manage all analysis & custom definitions created by others |
| **Feature Flags & Experimentation** | Create experiments, create feature flags, manage feature flags created by others |
| **Data Management** | Edit metadata, set event & property visibility, data verification, manage schemas, manage lookup tables, mark sensitive data, edit profiles, data deletion/dropping/merging, data settings access (volume monitoring, quality monitoring, event approval, data standards) |
| **Data Access, Exports & Alerts** | Public boards, sensitive data access, export data from Mixpanel, create alerts & webhooks, create board subscriptions, manage all alerts |
| **Data Sources** | Create & manage cohort syncs, create & manage pipelines, create & manage data warehouses |
| **Settings** | View & edit project settings (workspaces, service accounts, project users, project info, security settings, usage analytics) |

<Callout type="info">
All custom roles automatically include base permissions: viewing reports, editing visibility of entities you created, and locking/unlocking entities you created. These permissions cannot be removed.
</Callout>
...
1. Under "Project Settings", click **Project Users**.
2. Select the **Custom Roles** tab.
3. Find the role you want to edit and click the overflow menu (three dots) on the role card.
4. Select **Edit** to modify the role name, description, or permissions.
5. Click **Save** to apply your changes.

Changes to a custom role take effect immediately for all users assigned to that role.

### Cloning a Custom Role

You can clone a custom role to quickly create a similar role within the same project or copy it to another project.

To clone a custom role:

1. Under "Project Settings", click **Project Users**.
2. Select the **Custom Roles** tab.
3. Find the role you want to clone and click the overflow menu (three dots) on the role card.
4. Select **Clone** and choose whether to clone within the current project or to another project.
5. If cloning to another project, select the target project.
6. Modify the name, description, or permissions as needed.
7. Click **Save** to create the cloned role.
...
1. Under "Project Settings", click **Project Users**.
2. Select the **Custom Roles** tab.
3. Find the role you want to delete and click the overflow menu (three dots) on the role card.
