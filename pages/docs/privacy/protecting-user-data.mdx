...
Mixpanel's Web and Mobile libraries use IP address to enrich events with geographic information like city, country, and region. Mixpanel does not store IP addresses, but rather, only uses IPs to assign [geolocation properties to data upon ingestion](/docs/data-structure/property-reference#ingestion-apis). You can disable this using the following configuration options in each of our SDKs:
