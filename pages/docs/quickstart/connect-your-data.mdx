...
If you do not see an SDK or an integration in your language, you can send events to our HTTP API directly.
...
We recommend [setting up a proxy server](/docs/tracking-methods/sdks/javascript#tracking-via-proxy) so that you don't lose events due to ad-blockers.
...
To preserve battery life and customer bandwidth, the Mixpanel library doesn't send the events you record immediately. Instead, it sends batches to the Mixpanel servers every 60 seconds while your application is running, as well as when the application transitions to the background. You can call `.flush()` manually if you want to force a flush at a particular moment for example before your application is completely shutdown. See our [debugging guide](/docs/debugging/overview).
...
Please refer to our guide on [Identifying Users](/docs/tracking-methods/identifying-users) for a detailed walkthrough and example user identity flows.

### Client-side tracking

When using our client-side SDKs (Web or Mobile), Mixpanel will automatically generate a distinct_id value that's local to that user's device. There are only 2 steps:
...
When using our server-side SDKs (Python, Ruby, etc.), user identification is handled by your server which means you will need to generate your own distinct_id value for each event.
