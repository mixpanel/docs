...
When you see a "cardinality limit" error or a "-" in your report despite having data, it usually means the query returned too many unique segments for the UI to display. To avoid this, try narrowing the time range, applying filters to specific events or properties, or reducing the number of breakdowns. This helps limit the number of segments, making your report more accurate and easier to interpret.
