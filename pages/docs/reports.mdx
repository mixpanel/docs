...
A report is the basic unit of performing an analysis in Mixpanel. A report can be saved to a [Board](docs/boards/), and a board can contain many reports. Each report is made up of at least one **metric** and is displayed using a **visualization**.

Mixpanel has four core reports, along with other advanced apps.
