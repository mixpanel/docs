...
- You cannot nest a formula metric when constucting another formula. e

#### Save formulas for re-use

Use existing events to create calculated formulas, like Bounce Rate & % Active Users, that can be saved and reused by others within the project. This in turn enables everyone use the same ‘blessed’ metrics and removes uncertainty if the formula metric is the correct calculation.
