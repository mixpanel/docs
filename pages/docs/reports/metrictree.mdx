...
It’s the one space depicting both the quantitative data and qualitative data via context, making it possible to discuss insights as a team, across Execs to ICs, and take data backed decisions.

![image](https://github.com/mixpanel/docs/blob/562dc183d96bbaeec6cc395a6b4684320437e3f4/public/metrictree1.png)

# How to get started?

## Step 1 : Create your Metric Tree outline without data  

Idea is to think through your strategy, and the various hypothesis without confusing it with data available today or current value of metrics. Your strategy is more long term. Use this strategy map to discuss and align with your team-mates before putting in the effort to connect to a metric with data. To create a metric tree outline
...
![image](https://github.com/mixpanel/docs/blob/mmedina3-patch-1/public/metrictree2.png)

## Step 2: Make the metrics real with data 
Click on connect metric, and connect metric card to metric with data. This is a regular Mixpanel query panel. You can choose to bring in saved metrics, or create a metric. 

**NOTE:** We highly recommend saving metrics in the metric tree, especially those on the higher levels, so others on the project use only this standardized verified metrics. Saved Metrics has additional ability like adding owners to it.

![image](https://github.com/mixpanel/docs/blob/mmedina3-patch-1/public/metrictree3.png)

## Step 3: Add more context to your metrics 
Make your tree more actionable for yourself and the team by adding context to it - whether it is pinned content with some interesting deep dive reports, or looking at correlation and finding some interesting insights to add to the logbook. You can learn more [here.](https://docs.mixpanel.com/docs/reports/metrictree#Customize-each-metric)
...
Metric Tree is a multi-player space meant for collaboration and for you to use during your weekly team rituals. So many of you can be on the tree at the same time and edit it, and most importantly add insights or actions for metrics you’re accountable for.  
...
- Data driven decisioning: Clearly articulate what’s working vs not by looking at how your input metrics and initiatives are impacting your top business outcomes
...
- Use this as a plan to ensure you have the right data tracked : Use this as a guide to let you know which additional data needs to be tracked, or to ensure it’s modeled in a right way to define the metric
...
For example: if I wanted a QoQ view instead of YoY, I’d select 

- Time range: Last 3M
- Compare to: Previous quarter
- Interval: week; Interval determines the number of data points you have displayed in the metric line chart

![image](https://github.com/mixpanel/docs/blob/mmedina3-patch-1/public/metrictree4.png)
...
- On the metric card view, when you click on MoM, it shows the absolute value of the metric for the last 30 D. If you switch and click on QoQ it will show you the absolute value of the metric for the last 3M
- If you switch the default “display” on the metric card, all the other cards that have that time range will also switch so you compare metrics across the tree in a similar period.

## Add owner to keep a person accountable for the metric
For any eligible metric, you can click on the owner icon and assign owner 

**NOTE:** You can assign an owner only to a [saved metric](https://docs.mixpanel.com/docs/features/saved-metrics-and-behaviors#saved-metrics) 

![image](https://github.com/mixpanel/docs/blob/mmedina3-patch-1/public/metrictree5.png)
...
![image](https://github.com/mixpanel/docs/blob/mmedina3-patch-1/public/metrictree6.png)
...
![image](https://github.com/mixpanel/docs/blob/mmedina3-patch-1/public/metrictree7.png)

# FAQ

## What does `As Of` control on the top left of the metric tree do? 
`As of` determines the end date reference point for the metric tree. 

For example if you want to look at how the metric tree looked at the end of Q1, i.e March 2025, you could set `As of 31 March 2025` . All the metrics in the metric tree which are based on last 30 D, or last 3M etc, will look at measuring the value of last 30D as of 31 March 2025 i.e 2-31 March 2025 instead of last 30D as of today.
![image](https://github.com/mixpanel/docs/blob/mmedina3-patch-1/public/metrictree8.png)

## What does Filtering a tree do? 
Filtering a tree, is similar to filtering a board. It filters every metric in the metric tree. If you’re looking to filter only a specific metric, please use [inline filters.](https://docs.mixpanel.com/docs/reports/funnels/funnels-advanced#inline-filtering-pre-query) 

## How do I see the metric trend chart, i.e metric time series? 
...
No it doesn’t have to. We generally ask people to start a tree at L2 or L3 level where there is more functional ownership, example Marketing can own # Sign-Ups or Product can own WAU (Weekly Active Users) 
...
![image](https://github.com/mixpanel/docs/blob/mmedina3-patch-1/public/metrictree9.png)
...
For two metrics, **A** and **B**, that each return time-series data, we compute Pearson correlation using the following formula:
...
- **Some specific aspects to call out:**
    1. If 2 time-series data have different lengths, correlations is calculated based on the overlapping lengths between the 2 
    2. All correlations are run on daily granularity, since event data is available on a daily basis to compute metrics at that granular view.
