...
#### How does record_heatmap_data work with Autocapture and billing?

**Important: Events generated by `record_heatmap_data` are free only when a session recording is taking place.** Clicks generated by `record_heatmap_data` are exempt from your plan data allowance when a session recording is in progress. When `record_heatmap_data` is enabled, autocapture events will also be captured. These autocapture events are free only when a session recording is taking place. Otherwise, autocapture events are billable and counted against your plan data allowance. For more details, see our [Heatmaps documentation](/docs/session-replay/heatmaps).
