...
With UI Controls, teams can manage Session Replay sampling directly from the Mixpanel UI — no code changes or redeployment required. This is available exclusively to customers with the paid Session Replay Add-On. Changes made in the UI take effect almost immediately.

### Baseline Sampling Rate

Set a global sampling rate that applies to all sessions. This is the default capture rate before any conditional rules are applied. For example, you might set a baseline of 1% to capture a small portion of all traffic.
...
For example, you can sample 1% of all sessions globally, but capture 100% of sessions where `checkout_error` fires or where users land on your pricing page.
