...
Session Replay is an invaluable tool for teams to debug and understand user behavior, but it introduces inherent risks — especially in mobile environments. These risks are not unique to Mixpanel; they are common across the entire session replay category and industry.

Inherently, session replay captures what users see on their screens, which may include sensitive information. Mobile implementations carry additional considerations:
* **SDKs are installed directly on end-user devices**, making them harder to roll back or hotfix if issues arise.
* **Android's fragmented ecosystem**—with many device and OS combinations—increases the likelihood of edge cases and unexpected behavior.

With the right planning and safeguards, teams can unlock the value of replay while minimizing potential downsides.

### Categories of Risk

| **Risk Category**                  | **Description**                                                                                            |
| ---------------------------------- | ---------------------------------------------------------------------------------------------------------- |
| **Fatal Crashes**                  | The SDK may cause the app to crash during initialization, particularly on specific devices or OS versions. |
| **Masking Bugs**                   | Correctly configured masking rules may still fail due to SDK bugs, potentially exposing sensitive data.    |
| **Faulty Masking Implementations** | Developers may forget to / improperly mask sensitive screens or elements, leading to unintended data capture in production.         |

### Sensitive Areas to Watch

| **Sensitive Area**             | **Why It Matters**                                                                   |
| ------------------------------ | ------------------------------------------------------------------------------------ |
...
| **Embedded Webviews**          | Web content may behave unexpectedly with masking rules.                         |
| **User Inputs**                | Form fields and search bars can capture personal or confidential data.               |
| **Uncommon App Architectures** | Custom or non-standard implementations may introduce unexpected replay behavior.     |

---

To mitigate these risks, we recommend:
* **Defensive programming**, such as safe initialization and fallback behavior
* **Thorough QA**, including testing across device types and sensitive screens
* **Staged rollouts** to detect and address issues before full deployment

Session Replay is a powerful feature that should be implemented with care. Following best practices can help you gain valuable insights into user behavior while reducing potential risks to user trust or app stability.
...
- **SR SDK causing my app to crash**
...
**If the SR SDK is causing your app to crash:**
