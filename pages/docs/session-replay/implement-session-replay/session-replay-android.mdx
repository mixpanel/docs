...
## Best Practices
Session Replay provides powerful insights into user behavior, but it also introduces inherent risks—especially on mobile. These risks are not unique to Mixpanel; they are common across the entire session replay product category. Because SDKs run on end-user devices and screen content may include sensitive data, we recommend treating Session Replay as a high-sensitivity feature. Be especially cautious with masking, edge-case testing, and rollout strategies. For more information on risk categories and best practices, read more [here](/docs/session-replay#best-practices)
...
- Android Session Replay requires a minimum version of 26, and older versions are not officially supported at this time, so thorough testing on your supported OS versions is recommended.
