...
Recording only starts if the sampling check passes and you explicitly call startRecording(). Recording does not start automatically. Note: this is planned to change in upcoming releases, as we move towards recording beginning automatically if the sampling check passes.
...
Replay recording does not start automatically — you must explicitly call `.startRecording()` after the random sampling check passes.
