...
| `record_mask_text_selector` | `string \| string[]` | `""` | CSS selector(s) for elements to mask. Only applies when `record_mask_all_text` is `false`. |
| `record_unmask_text_selector` | `string \| string[]` | `""` | CSS selector(s) for elements to unmask. Only applies when `record_mask_all_text` is `true`. |
