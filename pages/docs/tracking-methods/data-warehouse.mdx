...
We add couple of hidden properties `$warehouse_import_id`  and `$warehouse_type` on every event ingested through warehouse connectors. You can add filters and breakdowns on that property in any of the mixpanel reports. You can find the Warehouse import id  of a sync in Sync History tab shown as `Mixpanel Import Id`.
