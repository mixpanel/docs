...
Mirror takes Redshift table snapshots and runs queries to compute the changes between two snapshots using hash value for each row. Snapshots are stored in a staging schema created in [Step 1](#step-1-connect-a-warehouse).
