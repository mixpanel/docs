...
Mirror syncs from Snowflake use [Snowflake streams](https://docs.snowflake.com/en/user-guide/streams-intro) to track changes to source tables.
[`CHANGE_TRACKING`](https://docs.snowflake.com/en/sql-reference/sql/alter-table) must be enabled on a Snowflake table in order for a Mirror
sync to be created from it.
```jsx
ALTER TABLE <database>.<schema>.<table> SET CHANGE_TRACKING = TRUE;
```

Mixpanel creates and manages Snowflake streams in the warehouse and needs the `CREATE STREAM` permission on a Snowflake schema it can use
to store these streams. We recommend creating a new empty schema solely for Mixpanel. For example:
```jsx
...
To create a new Mirror-ready Snowflake source follow the steps above and during step 4 fill out the additional two fields "Stream Database" and "Stream Schema".
