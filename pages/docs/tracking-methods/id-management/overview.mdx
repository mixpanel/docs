...
Identity management is the process of assigning IDs and maintaining ID mappings to ensure your data are attributed to you users accurately. Mixpanel uses the Distinct ID property to identify your users and determine their uniqueness.

## Distinct ID

Mixpanel uses the Distinct ID property to keep track of who is doing each. Every event data and profile update sent to Mixpanel should have a distinct_id property associated with it (except in the case of preventing [hot shards](/docs/debugging/distinct-id-limits)). The Distinct ID value tells Mixpanel which user to attach your events and profile property updates.
...
Below highlights a non-exhaustive list examples of Distinct ID's role in Mixpanel:
...
IMAGE OF NO ID MANAGEMENT FLOW
