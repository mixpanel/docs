...
2. Under Feature Flags, 
  ...
  b) If your flags use a data group key as a custom Variant Assignment Key (e.g., `company_id`) or Runtime Targeting, you'll need to pass a context object. Since GTM's key-value interface only supports primitive values, create a **Custom JavaScript Variable** that returns the flags config object:
...
async function() {
  var fallback = false;
  return await mixpanel.flags.is_enabled("my-feature-flag", fallback);
}
```

**Experiment Flags or Dynamic Configuration Flags— get a variant value:**
...
async function() {
  ...
  return await mixpanel.flags.get_variant_value("my-feature-flag", fallback);
}
```

If the current user is in the rollout of the flag, an exposure event, `$experiment_started` will be tracked automatically to Mixpanel.
