...
Lastly, track your users' actions using [Segment's `track` API call](https://segment.com/docs/connections/spec/track/), which would trigger [Mixpanel's `track` API](https://developer.mixpanel.com/reference/track-event) and ingest an event in your project.
