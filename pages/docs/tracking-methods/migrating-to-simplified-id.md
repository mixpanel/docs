# Overview
Mixpanel has three versions of ID Management versions to date. Prior to March 2020, customers were on Legacy ID Management system. 
In March 2020, we released Original ID Merge system which supports retroactive ID Merge. This feature allows the merging of events triggered both before and after authentication across multiple devices and platforms.
In March 2023, we released the current Simplified ID Merge system. It requires a simpler implementation and doesn’t have the limitation of 500 IDs per ID cluster found in the Original ID Merge system. 
To determine your current ID Management version, navigate to `Organisation/Project Settings > Identity Merge`. Organisation Settings indicate the default ID Management version for every new project created. You can toggle the ID Management version for a specific project via Project Settings if no data has been ingested into that project yet. Here are the 3 versions of ID Management:

# Deciding When Migrating Makes Sense
It is not possible to convert an existing project using Legacy/Original ID Merge to Simplified ID Merge. To adopt Simplified ID, you need to create a new project from scratch. This guide aims to help you in evaluating whether the migration will benefit your project based on your current ID management requirements and future plans. It outlines the pros and cons of each ID Management system and guides you through key considerations to make an informed decision. It also provides details on the tasks and resources required on your end should you decide to proceed with the migration. 
...
If you are solely tracking authenticated users (you do not track anonymous events and do not need the retroactive ID Merge feature in Simplified ID Merge for users merging), you are not running into any limitation in this system and should not consider the migration. We have preserved the documentation on the Legacy ID Management here. 

## Customers on Original ID Merge
While retroactive ID Merge is supported in Original ID Merge, the main limitation of this system is that each user can have a ID cluster limited to a maximum of 500 IDs. Upon reaching this limit, any new Distinct ID cannot be merged into the existing ID cluster. These orphaned IDs become duplicate users on Mixpanel, preventing you from getting a holistic view of the user journeys, as events from the same user are split among multiple users on Mixpanel.
...
That being said, if some of your users are approaching this cluster limit, you should revisit your implementation and consider removing the `.reset()` call, unless there is a compelling use case where the benefits outweigh the implications of reaching the ID cluster limit. 

Also, if you are considering Simplified ID Merge, it's important to note that it does not support multiple identified IDs (e.g. User IDs) per ID cluster. This is possible on Original ID Merge via special events such as `$merge` and `$create_alias` but they have been removed from Simplified ID Merge for simplicity.
...
- (Internal) [Here’s](https://mixpanel.com/project/3129045/view/3643217/app/profile#distinct_id=charlie&) the user on Mixpanel.
...
2. If you are sending events via third-party integrations, ensure that they are compatible with Simplified ID Merge by including reserved properties, `$device_id` and `$user_id` on the events. In terms of backward compatibility, Simplified ID still supports events with only the `distinct_id` property.  
    ...
    (Note that `distinct_id` will be overridden if either `$device_id` or `$user_id` exists on the event.)
...
1. If you are implementing Mixpanel in your mobile apps, you’ll need to ship a new version of the mobile app with the updated ID management implementation and new project’s token as part of the migration process. Without a force update, it may take awhile for all users to upgrade to the latest app version. During this period, some events may still be sent to the old project. Be prepared for data backfilling if you want these events, as well as the historical data to be included in the new project.
2. With the introduction of the retroactive ID Merge feature in Original and Simplified ID Merge, it may take up to 24 hours for the ID Merge (merging 2 unique users into 1 unique user) to be fully reflected in all Mixpanel reports.
