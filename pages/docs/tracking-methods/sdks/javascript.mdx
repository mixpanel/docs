...
We give our customers full control to customize when and where they capture session replays. Session Replay must be enabled via the `record_sessions_percent` init option [described above](/docs/tracking-methods/sdks/javascript#implementation--sampling) before these methods are used.
