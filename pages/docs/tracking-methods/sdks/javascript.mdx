...
For any element on your website / web application you do not want to track, there are build-in selector options to omit it. You can add any element to the `.mp-no-track` class to omit it. You can also opt an individual element out of being included in any tracking (including the `$elements` prop of another target's event) using `.mp-sensitive`.
...
| `pageview` | When set, Mixpanel will collect pageviews all components of the URL — including UTM parameters. | `"full-url"` |
...
| `allow_url_regexes` | Opts in specific pages from Autocapture. | `[]` |
| `block_selectors` | Opts out specific classes from Autocapture. | `[]` |
| `allow_selectors` | Opts in specific classes from Autocapture. | `[]` |
| `block_attrs` | Opts out specific attributes from Autocapture. | `[]` |
| `allow_element_callback` | A user-provided function that determines whether a specific element should be tracked via Autocapture or not. The function receives the element as its only argument, and should return `true` if the element should be tracked (otherwise the element will NOT be tracked). | `not set` |
| `block_element_callback` | A user-provided function that determines whether a specific element should be blocked from tracking via Autocapture or not. The function receives the element as its only argument, and should return `true` if the element should be blocked. | `not set` |

**HTML attributes captured by default**

By default, Autocapture collects the following HTML attributes on Autocaptured events:
...
**Default event properties on Autocaptured events**

On Autocaptured events, Mixpanel default-tracks the following properties:
| Event Property | Description |
| --- | --- |
| `$event_type` | The type of event that was captured (e.g., "click", "submit"). |
...
- Search for Session Recording Checkpoint events in your project, tracked as `$mp_session_record`. When you capture Mixpanel session replays, the SDK will automatically emit this default event. If you've implemented correctly, you should see these events appear. Note: the Session Recording Checkpoint event does not count as a billed event for events plans in Mixpanel.
