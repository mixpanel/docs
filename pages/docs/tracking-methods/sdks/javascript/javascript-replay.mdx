...
1. Ensure you have the [Mixpanel JavaScript SDK installed](/docs/tracking-methods/sdks/javascript#installing-the-library) in your application with Session Replay enabled.
...
**Note**: You must still enable Session Replay in your Mixpanel JavaScript SDK (for example by setting `record_sessions_percent` in `mixpanel.init`), and this client-side sampling configuration should align with your mParticle Session Recording Percentage setting.

**Example Usage**

```javascript Javascript
mixpanel.init('MIXPANEL-PROJECT-TOKEN', {
	record_sessions_percent: 10  // Configure this to match your mParticle connection settings
});
```

#### Manual Implementation (Advanced)

If you need more control over the integration, you can manually patch mParticle's methods to include session replay properties. This approach requires you to bind mParticle's device_id to Mixpanel's device_id and patch [logPageView()](https://docs.mparticle.com/developers/sdk/web/core-apidocs/classes/mParticle%20&%20mParticleInstance.html#method_logPageView) and [logEvent()](https://docs.mparticle.com/developers/sdk/web/core-apidocs/classes/mParticle%20&%20mParticleInstance.html#method_logEvent) methods.
