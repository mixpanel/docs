...
<Callout type="info">
    The python SDK server-side SDK is currently in Beta.
</Callout>
...
- You are a Mixpanel ENT customer and have the appropriate version of the SDK installed (minimum supported version is [`v5.0.0b2`](https://github.com/mixpanel/mixpanel-python). If not, please follow [this doc](/docs/quickstart/install-mixpanel) to install the SDK.
...
For remote evaluation, the SDK will make a network call to Mixpanel servers at assignment time. This mode is recommended for use cases where  
...
- Polling can be kicked off in two ways
    - `start_polling_for_definitions` - This will kick off polling in a background OS thread.
    - `astart_polling_for_definitions` - This will kick off polling in an asyncio task.
