...
The React Native SDK is a tracking SDK designed for real-time tracking in a client-side environment. Calling `track:properties:` triggers a request to our [/track API endpoint](https://developer.mixpanel.com/reference/track-event), which will validate for events with a timestamp that is within the last 5 days of the request. **Events older than 5 days will not be ingested**.
...
A few commonly used people methods are highlighted below:
  ...
  * [`.getPeople().setOnce()`](https://mixpanel.github.io/mixpanel-react-native/People.html#setOnce) - set profile properties only if they do not exist (will not overwrite existing properties)
  * [`.getPeople().append()`](https://mixpanel.github.io/mixpanel-react-native/People.html#append) - append values to a list profile property
  * [`.getPeople().union()`](https://mixpanel.github.io/mixpanel-react-native/People.html#union) - append to a list property, excluding duplicates
  * [`.getPeople().increment()`](https://mixpanel.github.io/mixpanel-react-native/People.html#increment) - increment/decrement numeric profile properties
...
mixpanel.getPeople().set_once('name', 'Samantha');

// "email" will be set since it does not exist
mixpanel.getPeople().set_once('email', 'test@mail.com');
...
A few commonly used group methods are highlighted below:
  * [`.getGroup().setOnce()`](https://mixpanel.github.io/mixpanel-react-native/MixpanelGroup.html#setOnce) - set group properties only if they do not exist (will not overwrite existing properties)
  * [`.getGroup().unset()`](https://mixpanel.github.io/mixpanel-react-native/MixpanelGroup.html#unset) - remove a group property from a group profile
  * [`.getGroup().union()`](https://mixpanel.github.io/mixpanel-react-native/MixpanelGroup.html#union) - merge a list with a list-valued group property, excluding duplicates
  * [`.getGroup().remove()`](https://mixpanel.github.io/mixpanel-react-native/MixpanelGroup.html#remove) - remove a value from a list-valued group property
