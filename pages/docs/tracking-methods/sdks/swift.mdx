...
CocoaPods support for the Mixpanel SDK will be discontinued before the end of 2026, in line with the [CocoaPods platform deprecation](https://blog.cocoapods.org/CocoaPods-Specs-Repo/) — the CocoaPods trunk is going permanently read-only on December 2, 2026, making it impossible to publish new SDK versions through that channel. We recommend migrating to Swift Package Manager (SPM), which is already fully supported and will be the only supported installation method going forward. See the SPM installation instructions above to get started.
