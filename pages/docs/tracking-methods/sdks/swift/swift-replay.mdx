...
You can use the `getReplayId()` method to return the [Replay ID](/docs/session-replay/implement-session-replay/session-replay-ios#replay-id) for the current replay recording. The method will return an empty object if there is no active replay capture in progress.
