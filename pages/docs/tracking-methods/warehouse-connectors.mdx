...
| **Historical events** | Not billed | If you send in historical events for a past month, it will not count towards billing |
| **Event table updates (Mirror)** | Billed when update occurs | If you already have an event in Mixpanel, for example, Event A with properties a, b, c, d but want to: <br/>• update the value of property d, or<br/>• add another property e<br/>this will count as a billable event |
| **Event table column additions (Mirror)** | Billed for each row for non-NULL column values | Adding a column will generate billable events for each row in the affected table |
| **Event table deletes (Mirror) ** | Billed when the delete occurs | Deleting events or properties will generate billable events for each affected row |
...
- You want to minimize billable operations (as updates and deletes don't count)
