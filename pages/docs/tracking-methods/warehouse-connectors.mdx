...
## Warehouse Connectors Billing FAQ

### How am I billed for Mixpanel when using a warehouse connector?

When using a warehouse connector, you are billed based on the amount of data imported to Mixpanel through the connector. Each event sent from your warehouse to Mixpanel counts as one data point, just like events sent through our other tracking methods.

Your organization will be charged according to your plan's data point pricing. For more information, please visit our [pricing page](https://mixpanel.com/pricing/).

### How do different actions impact my Mixpanel billing when using warehouse connectors?

When using warehouse connectors, different types of operations have varying impacts on your billing. The table below explains how each action affects your monthly event volume:
...
| **Historical events** | Not billed | If you send Event A with properties a, b, c, d as a historical event (from a past month), it will not count toward billing |
| **Event table updates** | Billed for the month the update happens | If you already have Event A in your database and add property e or update property b's value, each affected row counts as a billable event |
...
You can track your data usage in Mixpanel by navigating to Organization Settings > Plan Details and Billing. Head to Data Usage > View Historical Usage to view event usage by project and by Mirror.


### How do different actions impact my DWH billing?
