...
<Callout type="info">
  Customers on an Enterprise can access Warehouse Connectors as an add-on package. See our [pricing page](https://mixpanel.com/pricing/) for more details.
</Callout>
...
<Callout type="warning">
  Redshift Mirror is currently in beta.
</Callout>
...
- `_mp_row_hash`, `_mp_change_type` are reserved column names used for tracking the computed hash value for each row, and update type to the row. Please ensure that this doesn’t conflict with the column names  of the table.
...
### How do different actions impact my billing when using warehouse connectors?
...
| Action | Impact on Billing | Example |
|--------|-------------------|---------|
| **New events in current month** | Counts as billable events | If you sync events that occurred this month, each event counts toward your monthly quota |
| **Historical events** | Not billed | If you already have an event in your Mixpanel, for example, Event A with properties a, b, c, d, and you send in historical events for a past month, it will not count towards billing |
| **Event table updates** | Billed for the month the update happens | If you already have an event in Mixpanel, for example, Event A with properties a, b, c, d but want to: <br>• update the value of property d, or<br>• add another property e<br>this will count as a billable event |
| **Event table column additions** | Only billed for non-NULL values | Adding a column with NULL values does not count as an update across all supported warehouses |
...
| Mode | Operation | Impact on Billing |
|------|-----------|-------------------|
| **Standard mode (Append)** | Inserts, updates, deletes | Not billed |
| **History mode (Mirror)** | New profiles in current month | Billed like regular events |
| **History mode (Mirror)** | Historical profiles | Not billed for past months, similar to events |
| **History mode (Mirror)** | Updates and deletes | Billed when the operation occurs |

**Note**: Our Mirror functionality looks at the current data in tables and uses different approaches (depending on the platform) to track changes to only perform operations based on those changes. This is important to consider for your data modeling process. If your workflow consists of dropping a table and recreating it with the new version of the data, Mirror would most likely consider everything as deleted and new, which would significantly influence billing. However, if your workflow keeps the previous version of the data and just appends, updates, and deletes rows, billing will be based only on the new operations as the tables above describe.
...
- You need to maintain an exact replica of your warehouse data in Mixpanel
...
- You want to minimize billable operations (as updates and deletes don't count towards billing)
