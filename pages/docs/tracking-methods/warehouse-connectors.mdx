...
| **Data Type** | **Operation**                            | **Sync Type** | **Supported** | **Billing** |
|---------------|-------------------------------------------|---------------|---------------|-------------|
| **Events**    | **New events (current calendar month)**   | Append        | ✅            | ✅          |
|               |                                           | Mirror        | ✅            | ✅          |
|               |                                           | One time      | ✅            | ✅          |
|               | **Historical events (backfills)**         | Append        | ❌            | ❌          |
|               |                                           | Mirror        | ✅            | ❌          |
|               |                                           | One time      | ✅            | ❌          |
|               | **Updates & deletes (see below)**         | Append        | ❌            | ❌          |
|               |                                           | Mirror        | ✅            | ✅          |
|               |                                           | One time      | ❌            | ❌          |

*Updates & deletes: Examples *

If you already have an event in Mixpanel, for example, Event A with properties a,b,c,d but want to:

- Update the value of property d, or
- Add another property e with a non-NULL value

This will count as a update event for each row in the sync.
