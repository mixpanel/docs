...
|---------------|-------------------------------------------|---------------|---------------|-------------|
| **Events**    | **New events (current calendar month)**  | One time      | ✅            | ✅          |
|               |                                           | Append        | ✅            | ✅          |
|               |                                           | Mirror        | ✅            | ✅          |
...
- Add a new property e with a non-NULL value

This will count as an update event for each row in the sync.

More info: Our Mirror functionality looks at the current data already uploaded in Mixpanel and uses different approaches (depending on the warehouse) to identify changes performed within your warehouse platform. This is important to consider for your data modeling process within your warehouse. If your warehouse workflow consists of dropping a table and recreating it with a new version of the data, Mirror would idenify this change as ingesting the entire table deleted with each row deleted as a billable event, and the entire table recreated as a new billable event. However, if your workflow keeps the previous version of the data and just appends, updates, and deletes rows, billing will be based only on the new operations as the table above describes.
...
**Note on Backfills**: If you already have an event in your Mixpanel project, and you sync in historical events for a past month with the same properties, it will not count towards billing. Otherwise, billing will occur at the time of the event update. Please contact your Account Manager for guidance on historical backfills that may affect billing.

**Note on Mirror**: 

**Billing for User/Group Profiles Syncs**

| **Data Type**           | **Operation**                      | **Mode**     | **Sync Type**     | **Supported** | **Billed** |
|-------------------------|------------------------------------|--------------|-------------------|---------------|-------------|
| **User/Group Profiles** | **Set/update profile properties**  | Standard     | Full sync         | ✅            | ❌          |
|                         |                                    | Standard     | Append            | ✅            | ❌          |
|                         |                                    | Historical   | Mirror            | ✅            | ✅          |
|                         | **Unset profile properties**       | Standard     | Full sync         | ❌            | ❌          |
|                         |                                    | Standard     | Append            | ❌            | ❌          |
|                         |                                    | Historical   | Mirror            | ✅            | ✅          |
