...
You can use Mixpanel's Server SDKs to send events from your backend servers to Mixpanel. We [recommend](/docs/getting-started/plan-your-implementation#need-to-start-tracking-product-data) server-side tracking, since it is more reliable and easier to maintain than web/mobile tracking.
...
func main() {
	...
	mp := mixpanel.NewClient("PROJECT_ID", "PROJECT_TOKEN", "PROJECT_SECRET")
	if err := mp.Track(ctx, []*mixpanel.Event{
		mp.NewEvent("My New Event", "", map[string]any{
			"Signup Type": "Referral",
		}),
	}); err != nil {
		panic(err)
