...
Similar to events, cohorts can be verified to flag it as trustworthy. Simply select the cohorts in the **Lexicon > Cohorts** table by checking them and verify up to 100 cohorts at a time. You can perform the same action by opening the overflow menu with the "..." on the right, or clicking on the empty verified icon in the edit modal. You need to have verification permissions and edit permissions on the cohort to verify the cohort.

By labeling a cohort as verified, the verification badge will be displayed on the cohorts table which shows the person & date of the verification, as well as allows for users to filter by verification status in the table. In query builders, an identical verified badge is displayed for a verified cohort, and they will appear under the "Verified" section.

Additionally, you can un-verify a cohort by following the same steps for an already verified cohort. 
