...
[Business Context](/docs/business-context) is a shared layer where your team defines what Mixpanel can't infer from raw events: key metric definitions, fiscal calendar, launch history, team structure. Every AI surface reads it automatically.

Without it, AI answers are generic. With it, Mixpanel Agent knows "activation" means a user completed a key value moment. The RCA Agent knows a drop on November 1 isn't anomalous — it's the day after a promo ended.

For setup instructions, see the [Business Context documentation](/docs/business-context). Write in plain language — anything an informed new hire would need.
...
**On alerts:** Before, an alert was the beginning of an investigation. Now pair every meaningful [alert](/docs/features/alerts) with RCA — when it fires, the investigation runs automatically and you get a confidence-weighted explanation before you've opened Mixpanel.
