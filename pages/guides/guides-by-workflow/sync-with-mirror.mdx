...
Mirror is one of several sync modes in [Warehouse Connectors](/docs/tracking-methods/warehouse-connectors#mirror). It uses Change Data Capture (CDC) to detect and replicate changes, which means every row update or deletion in your source counts as a billable event in Mixpanel. That makes data modeling decisions consequential — not just for accuracy, but for cost.

You'll need:
- Access to a supported warehouse: Snowflake, BigQuery, Databricks, or Redshift
