...
export default function WhatsNewPromo() {
  ...
  // -------- Horizontal scroller with fade, arrows, progress --------
  const Carousel = ({ items, tileWidth = 420, gap = 24 }) => {
    ...
    return (
      <div className="relative">
        ...
        <div
          ref={ref}
          className="relative flex overflow-x-auto gap-6 pb-10 snap-x snap-mandatory scroll-smooth"
          style={{
        ...
        >
          {items.map((i) => (
            <div key={i.url} className="snap-start shrink-0" style={{ width: tileWidth }}>
              <Tile item={i} />
            </div>
          ))}
        </div>
