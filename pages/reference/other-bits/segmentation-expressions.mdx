...
Segmentation Expressions are used in [Query API](https://developer.mixpanel.com/reference/query-api) and [Raw Data Export API](https://developer.mixpanel.com/reference/raw-data-export-api) to allow for more specificity when querying your data.
...
| `user["$email"] == "allison@example.com"` | Returns `true` if `$email` user property is `"allison@example.com"` otherwise `false`. |
