...
You can use Mixpanel's Server SDKs to send events from your backend servers to Mixpanel. We [recommend](/getting-started/plan-your-implementation#need-to-start-tracking-product-data) server-side tracking, since it is more reliable and easier to maintain than web/mobile tracking.

## Step 1: Install the SDK
<Tabs items={['Python', 'Node', 'Ruby', 'Java']}>
<Tab>
```shell Python
...
Don't see your language here? See all our libraries in [Github](https://www.github.com/mixpanel) and our references for [Python](/tracking/advanced/python), [Node](/tracking/advanced/nodejs), [Ruby](/tracking/advanced/ruby), and [Java](/tracking/advanced/java). We also have a simple [HTTP API](/tracking/http-api) for any languages we don't support.
