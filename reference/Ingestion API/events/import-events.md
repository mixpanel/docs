...
## Authentication
 ...
 `/import` requires an Owner or Admin [Service Account](https://developer.mixpanel.com/reference/service-accounts). project_id, service account username, and service account password are required to authenticate a request.

`/import` also supports [Project Token](https://developer.mixpanel.com/reference/project-token) as an authentication method. You can provide your token as the basic auth username value with an empty password. If project_id is not specified, the request will be authenticated using the provided token.
