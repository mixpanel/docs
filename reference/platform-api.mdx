---
title: Overview
description: "Manage Mixpanel organizations, projects, and their resources programmatically with the Platform API: regional base URLs, service account auth, and pagination"
---
...
The service account must have access to the organization, project, or workspace specified in the request path. Each endpoint's reference page lists the required permissions and the default roles that grant them. See [Service Accounts](/reference/service-accounts) to learn how to create a service account and manage its access.
...
Endpoints that return a collection are paginated with an opaque cursor.
