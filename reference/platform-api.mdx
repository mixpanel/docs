...
It is a separate group because it is one consistently designed surface rather than a set of independently grown endpoints. Every Platform API endpoint shares the same base path, authentication, pagination, error format, and versioning contract, and its reference documentation is generated from the same schemas that serve the requests. Mixpanel's older APIs predate those conventions and keep their own; new platform capabilities are added here.
...
These apply across every Platform API endpoint, so you can write them into a client once.
...
| --- | --- |
| `page_size` | Items per page, from 1 to 100. Defaults to 100. |
...
Mixpanel APIs support an optional, date-based [`Mixpanel-Version` header](/reference/api-versioning) that pins the contract your integration was built against, so a future breaking change doesn't reach code you aren't ready to update. Setting it is recommended for production integrations.
