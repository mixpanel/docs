...
<Warning>
  **Deprecation Warning**
  ...
  This authentication method has been deprecated and will be fully retired on **March 3, 2027**. Existing projects will continue to be able to use API Secret until that date. Newly created projects no longer have access to the deprecated authentication method. Please migrate to [Service Accounts](/reference/service-accounts) instead, as they are the recommended authentication mechanism going forward.
</Warning>
...
For projects that have an API Secret, it can be found in the Access Keys section of a project's settings overview page: [https://mixpanel.com/settings/project/](https://mixpanel.com/settings/project/). Newly created projects do not have an API Secret; use [Service Accounts](/reference/service-accounts) instead.
