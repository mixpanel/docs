---
title: Project Token
description: "Every Mixpanel project has a unique alphanumerical token for collecting data"
---
...
For [Track Events](/reference/track-event), tokens are provided as values inside of the data sent to Mixpanel. Where this value is stored is dependent on the API endpoint used. Information on the specific location can be found in the Ingestion API's specification.
...
For [Import Events](/reference/import-events), the token is provided in the header as the basic auth username value with an empty password.
