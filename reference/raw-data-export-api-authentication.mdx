---
title: Authentication
description: "Authenticate raw data Export API requests with a service account or project secret, including project ID requirements"
---

## Service Account

[Service Account Authentication Details](/reference/service-accounts)

Note that when you use Service Account authentication, you must specify the project\_id as a query parameter. E.g. `https://data.mixpanel.com/api/2.0/export?project_id=12345`

**Projects with [Classified Data](/docs/admin/data-governance/data-views-data-classification#data-classification)**

- The Service Account must have permission to access classified data.

## Project Secret

[Project Secret Authentication Details](/reference/project-secret)
