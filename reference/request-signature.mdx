...
description: "Deprecated request signature authentication for Mixpanel APIs and how to calculate a signature; migrate to service account authentication"
---
...
**api\_key**: This is the token from the corresponding to the project you wish to consume from. **api\_secret**: This is a secret API key corresponding to the project. **expire**: UTC time in seconds; used to expire an API request. **all other request parameters**: All of the information for the API request being performed

Calculating the signature is done in parts:

1. Alphabetically sort all parameters being used in the request. This excludes your api\_secret
...
3. append the api\_secret to the string of concatenated parameters.
