...
| `event["$time"] >= datetime("YYYY-MM-DDT00:00:00")` | Returns `true` if the `$time` event property is on or after the given date, otherwise `false`. Replace `YYYY-MM-DD` with the date you want to filter from. |
