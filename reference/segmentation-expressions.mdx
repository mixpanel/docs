...
description: "Expression syntax for filtering events in the Query and Raw Data Export APIs"
---

Segmentation Expressions are used in [Query API](/reference/query-api) and [Raw Data Export API](/reference/raw-data-export-api) to allow for more specificity when querying your data.
...
| `event["$time"] >= datetime("YYYY-MM-DDT00:00:00")` | For the Event Export API: Returns `true` if the `$time` event property is on or after the given date, otherwise `false`. Replace `YYYY-MM-DD` with the date you want to filter from. |
