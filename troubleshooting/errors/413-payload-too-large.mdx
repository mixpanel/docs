...
<Note>
  The byte count in this example comes from the API reference's sample response and doesn't match the documented 10MB request limit below. Check the limit stated in the error message you actually received.
</Note>
...
Each event must be smaller than 1MB of uncompressed JSON. This is a per-event validation requirement, so an oversized event may be reported in `failed_records` on a [400](/troubleshooting/errors/400-bad-request) rather than as a 413. Oversized events usually come from embedding a large payload in a property — a JSON blob stored as a string, a base64 attachment, or full page HTML.
