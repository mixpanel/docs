...
<Note>
  The API reference doesn't document a `Retry-After` header. Compute your own backoff interval using the strategy below.
</Note>
...
If you are an Enterprise customer and a one-time backfill needs more headroom, contact your account team (CSM or sales representative) with your `project_id` and the use case.
